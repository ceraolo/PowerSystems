...
package Ideal "Custom models"
  ...
  record SCparameter "Ideal semiconductor parameters"
    ...
    parameter Real[2] eps(
      final min={0,0},
      each final unit="1") = {1e-4,1e-4} "{resistance 'on', conductance 'off'}";
...
A generalisation to powers of i and v is straightforward.</p>
  ...
  equation
    V = if size(par.cT_loss, 1) == 0 then par.Vf else par.Vf*loss(T - par.T0_loss,
      par.cT_loss);
    {v,i_sc} = if on then {par.eps[1]*s + (1 - par.eps[1])*V,s - (1 - par.eps[2])
      *V} else {s,par.eps[2]*s};
    annotation (Documentation(info="<html>
</html>
"));
...
<p>Ideal Diode with forward threshold voltage <tt>Vf_d</tt>.</p>
      ...
      Icon(coordinateSystem(
          ...
          grid={2,2}), graphics={Polygon(
              ...
              lineColor={0,0,255},
              fillColor={0,0,255},
              fillPattern=FillPattern.Solid),Line(points={{-90,0},{-40,0}},
            color={0,0,255}),Line(points={{40,0},{90,0}}, color={0,0,255}),Line(
...
"),
      Icon(coordinateSystem(
          ...
          grid={2,2}), graphics={Polygon(
            ...
            color={0,0,255}),Line(points={{20,0},{90,0}}, color={0,0,255}),Line(
            points={{20,40},{20,-40}}, color={0,0,255}),Line(
...
(Equivalent to ideal GTO or IGBT).</p>
      ...
      Icon(coordinateSystem(
          ...
          grid={2,2}), graphics={Polygon(
              ...
              fillPattern=FillPattern.Solid),Line(points={{-90,0},{-60,0}},
            color={0,0,255}),Line(points={{20,0},{90,0}}, color={0,0,255}),Line(
            points={{20,40},{20,-40}}, color={0,0,255}),Line(points={{20,0},{60,
            40},{60,90}}, color={255,0,255})}));
  end SCswitch;
  ...
  equation
    V = if size(par.cT_loss, 1) > 0 then par.Vf*loss(T - par.T0_loss, par.cT_loss)
       else par.Vf;
    if gate then
      if s > V then
      ...
      {v,i_sc} = if s < -V then {par.eps[1]*s - (1 - par.eps[1])*V,s + (1 - par.eps[
        2])*V} else {s,par.eps[2]*s};
    end if;
    annotation (
      defaultComponentName="GTO_D1",
      Documentation(info="<html>
...
(Equivalent to ideal GTO or IGBT with reverse Diode).</p>
      ...
      Icon(coordinateSystem(
          ...
          grid={2,2}), graphics={Polygon(
              ...
              fillColor={0,0,255},
              fillPattern=FillPattern.Solid),Line(points={{-90,0},{-60,0}},
            color={0,0,255}),Line(points={{60,0},{90,0}}, color={0,0,255}),Line(
            points={{30,70},{30,10}}, color={0,0,255}),Polygon(
              points={{-30,-40},{40,-10},{40,-70},{-30,-40}},
              lineColor={0,0,255},
              fillColor={0,0,255},
              fillPattern=FillPattern.Solid),Line(points={{30,40},{60,40},{60,-40},
            {40,-40}}, color={0,0,255}),Line(points={{-40,40},{-60,40},{-60,-40},
            {-30,-40}}, color={0,0,255}),Line(points={{30,40},{60,70},{60,90}},
            color={255,0,255})}));
  end SCswitch_Diode;
