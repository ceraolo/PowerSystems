...
"),
  ...
  model Fault_abc "a to b to c fault, 3-phase dq0"
    ...
    replaceable Common.Switching.Short fault_pp1(
      final on=on,
      final epsR=epsR,
      final epsG=epsG) "fault model" annotation (choices(choice(redeclare
            PowerSystems.Common.Switching.Short fault_pp1
            "short with small resistance"), choice(redeclare
            PowerSystems.Common.Switching.Fault fault_pp1
    ...
    replaceable Common.Switching.Short fault_pp2(
      ...
      final epsR=epsR,
      final epsG=epsG) "fault model" annotation (choices(choice(redeclare
            PowerSystems.Common.Switching.Short fault_pp2
            "short with small resistance"), choice(redeclare
            PowerSystems.Common.Switching.Fault fault_pp2
  ...
  equation
    ...
    annotation (
      defaultComponentName="fault_abc",
      Documentation(info="<html>
<p>Connect to 'fault'-terminal of faulted line.</p>
</html>
"),
...
"),
  ...
  model Fault_Abc "b to a, c to a, a to ground fault, 3-phase dq0"
    ...
    annotation (defaultComponentName="fault_Abc", Documentation(info="<html>
<p>Connect to 'fault'-terminal of faulted line.</p>
</html>
"));
  end Fault_Abc;
...
"));
  ...
  package Partials "Partial models"
    ...
    partial model FaultBase "Line fault base, 3-phase dq0"
      ...
      parameter Integer n_phRef(
        min=1,
        max=3) = 1 "of reference phase" annotation (choices(
          choice=1 "phase a",
          choice=2 "phase b",
          choice=3 "phase c"));
...
"), Icon(coordinateSystem(
    ...
    partial model Fault_pp "Two-phase fault, 3-phase dq0"
      ...
      parameter Integer n_ph(
        min=1,
        max=3) = 1 "faulted pair" annotation (choices(choice(redeclare
              PowerSystems.Common.Switching.Short fault_pp
              "short with small resistance"), choice(redeclare
              PowerSystems.Common.Switching.Fault fault_pp
              "fault with arc-model")), Placement(transformation(extent={{-20,-20},
                {20,20}})));
...
<p>When actual fault is initiated, boolean variable on is set to true.</p>
    ...
    partial model Fault_pg "One-phase to ground fault, 3-phase dq0"
      ...
      parameter Integer n_ph(
        min=1,
        max=3) = 1 "faulted phase" annotation (choices(
          choice=1 "phase a",
          choice=2 "phase b",
          choice=3 "phase c"));
      replaceable Common.Switching.Short fault_pg(
        ...
        final epsR=epsR,
        final epsG=epsG) "fault model" annotation (choices(choice(redeclare
              PowerSystems.Common.Switching.Short fault_pg
              "short with small resistance"), choice(redeclare
              PowerSystems.Common.Switching.Fault fault_pg
              "fault with arc-model")), Placement(transformation(extent={{-20,-20},
                {20,20}})));
      //  extends Common.Switching.Partials.FaultBase
    protected
    ...
    partial model Fault_ppg "Two-phase to ground fault, 3-phase dq0"
      ...
      parameter Integer n_ph(
        min=1,
        max=3) = 1 "faulted pair" annotation (choices(
          choice=1 "phase bc",
          choice=2 "phase ca",
          choice=3 "phase ab"));
      replaceable Common.Switching.Short fault_pp(
    ...
    partial model Fault_pppg "Three-phase to ground fault, 3-phase dq0"
      ...
      parameter Integer n_ph(
        min=1,
        max=3) = 1 "which phase to ground" annotation (choices(
          choice=1 "phase a",
          choice=2 "phase b",
          choice=3 "phase c"));
      replaceable Common.Switching.Short fault_pp1(
        ...
        final epsG=epsG) "fault model" annotation (choices(choice(redeclare
              ...
              "short with small resistance"), choice(redeclare
              PowerSystems.Common.Switching.Fault fault_pp1
              "fault with arc-model")), Placement(transformation(extent={{-80,-20},
                {-40,20}})));
      //  extends Common.Switching.Partials.FaultBase
      replaceable Common.Switching.Short fault_pp2(
        final on=on,
      ...
      replaceable Common.Switching.Short fault_pg(
        ...
        final epsG=epsG) "fault model" annotation (choices(choice(redeclare
              ...
              "short with small resistance"), choice(redeclare
              PowerSystems.Common.Switching.Fault fault_pg
              "fault with arc-model")), Placement(transformation(extent={{40,-20},
                {80,20}})));
      //  extends Common.Switching.Partials.FaultBase
    protected
      final parameter Integer[2] m_ph=pair[n_ph, :];
