  ...
  equation
    if dynType <> Types.Dynamics.SteadyState then
      ...
      r = relaxation(
          ...
          t_relax,
          p_relax);
      {v,i} = if closed then {(r[1] + r[2]*epsR)*s,(r[1]*epsG + r[2])*s} else {
        (r[1]*epsR + r[2])*s,(r[1] + r[2]*epsG)*s};
    else
      t0 = 0;
      r = {1,0};
  ...
  equation
    ...
    if dynType <> Types.Dynamics.SteadyState then
      ...
      r = relaxation(
          ...
          t_relax,
          p_relax);
      {v_t,i_t} = if closed_t then {(r[1] + r[2]*epsR)*s_t,(r[1]*epsG + r[2])*
        s_t} else {(r[1]*epsR + r[2])*s_t,(r[1] + r[2]*epsG)*s_t};
...
"), Diagram(coordinateSystem(
            preserveAspectRatio=false,
            extent={{-100,-100},{100,100}},
            grid={2,2}), graphics={Line(points={{-80,-60},{0,-60}}, color={0,0,
              255}),Line(points={{60,-60},{80,-60}}, color={0,0,255}),Line(
              points={{-80,0},{-30,0}}, color={0,0,255}),Line(points={{30,0},{
              80,0}}, color={0,0,255}),Line(points={{-80,60},{-60,60}}, color={
              0,0,255}),Line(points={{0,60},{80,60}}, color={0,0,255})}));
