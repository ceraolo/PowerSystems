...
package Generation "Turbo generator groups dq0"
  ...
  equation
    ...
    annotation (
      defaultComponentName="turboGen1",
...
"),   Icon(coordinateSystem(
  ...
  model TurboGrpGenerator "Example turbogroup generator"
    ...
    replaceable model Torque =
        PowerSystems.Mechanics.TurboGroups.PcontrolTorque (final w_nom=
            turboGroup.par.w_nom, final P_nom=turboGroup.par.P_nom)
      "torque-model" annotation (choicesAllMatching=true);
    Torque turbTorq "torque-model"
      annotation (Placement(transformation(extent={{-60,-10},{-40,10}})));

    replaceable model TurboGroup =
        PowerSystems.Mechanics.TurboGroups.SteamTurboGroup (final w_start=
            w_start) "steam turbo-goup with generator-rotor" annotation (
        choicesAllMatching=true);
    TurboGroup turboGroup "steam turbo-goup with generator-rotor"
      annotation (Placement(transformation(extent={{-10,-10},{10,10}})));
  protected
    final parameter SI.Time h=(sum(turboGroup.par.J_turb) + turboGroup.par.J_gen
         + sum(turboGroup.par.J_aux))*w_nom^2/(2*generator.par.S_nom)
      "inertia cst turb + gen";
  ...
  equation
    ...
    connect(governor.power, turbTorq.power)
      annotation (Line(points={{-44,50},{-44,10}}, color={0,0,127}));
    annotation (
      defaultComponentName="turboGrpGen1",
      Documentation(info="<html>
<p>Turbo generator model with governor and exciter.<br>
...
"),   Icon(coordinateSystem(
  ...
  model GTGenerator "Example gas turbine generator"
    ...
    replaceable model Torque =
        PowerSystems.Mechanics.TurboGroups.PcontrolTorque (final w_nom=GT.par.w_nom,
          final P_nom=GT.par.P_nom) "torque-model" annotation (
        choicesAllMatching=true);
    Torque turbTorq "torque-model"
      annotation (Placement(transformation(extent={{-60,-10},{-40,10}})));

    replaceable model Gear = PowerSystems.Mechanics.TurboGroups.GasTurbineGear
        (final w_start=w_start) "gas turbine with gear and generator-rotor"
      annotation (choicesAllMatching=true);
    Gear GT "gas turbine with gear and generator-rotor"
      annotation (Placement(transformation(extent={{-10,-10},{10,10}})));
  protected
    final parameter Real[3] gr2=diagonal(GT.par.ratio)*GT.par.ratio/GT.par.ratio[
        end]^2;
    final parameter SI.Inertia J_red=(GT.par.J_turb + GT.par.J_comp)*gr2[1] +
        GT.par.J_gear1*gr2[1:2] + GT.par.J_gear2*gr2[2:3] + (GT.par.J_acc + GT.par.J_cpl
         + GT.par.J_gen) "gear reduced inertia";
    final parameter SI.Time h=J_red*w_nom^2/(2*generator.par.S_nom);

  equation
    assert(abs(2*pi*generator.par.f_nom/(generator.par.pp*GT.par.w_nom) - 1) <
      1e-3,
...
"),   Icon(coordinateSystem(
  ...
  model HydroGenerator "Hydro generator"
    ...
    replaceable model Torque =
        PowerSystems.Mechanics.TurboGroups.PcontrolTorque (final w_nom=hydro.par.w_nom,
          final P_nom={hydro.par.P_nom}) "torque-model" annotation (
        choicesAllMatching=true);
    Torque turbTorq "torque-model"
      annotation (Placement(transformation(extent={{-60,-10},{-40,10}})));
    ...
    replaceable model HydroTurbine =
      ...
      "hydro turbine with generator-rotor" annotation (choicesAllMatching=true);
    HydroTurbine hydro "hydro turbine with generator-rotor"
      annotation (Placement(transformation(extent={{-10,-10},{10,10}})));
  protected
    final parameter SI.Time h=(hydro.par.J_turb + hydro.par.J_gen)*w_nom^2/(2*
        generator.par.S_nom);

  equation
    assert(abs(2*pi*generator.par.f_nom/(generator.par.pp*hydro.par.w_nom) - 1)
       < 1e-3,
      "nominal rpm, frequency and pole-pair number seems to be incompatible");
    ...
    connect(governor.power, turbTorq.power)
      annotation (Line(points={{-44,50},{-44,10}}, color={0,0,127}));
    annotation (
      defaultComponentName="hydroGen1",
      Documentation(info="<html>
...
"),   Icon(coordinateSystem(
  ...
  model DieselGenerator "Diesel generator"
    ...
    replaceable model Torque =
        PowerSystems.Mechanics.TurboGroups.PcontrolTorque (final w_nom=diesel.par.w_nom,
          final P_nom={diesel.par.P_nom}) "torque-model" annotation (
        choicesAllMatching=true);
    Torque turbTorq "torque-model"
      annotation (Placement(transformation(extent={{-60,-10},{-40,10}})));
    ...
    Diesel diesel "Diesel engine with generator-rotor"
      annotation (Placement(transformation(extent={{-10,-10},{10,10}})));
  protected
    final parameter SI.Time h=(diesel.par.J_turb + diesel.par.J_gen)*w_nom^2/(2
        *generator.par.S_nom);

  equation
    assert(abs(2*pi*generator.par.f_nom/(generator.par.pp*diesel.par.w_nom) - 1)
       < 1e-3,
      "nominal rpm, frequency and pole-pair number seems to be incompatible");
    ...
    connect(governor.power, turbTorq.power)
      annotation (Line(points={{-44,50},{-44,10}}, color={0,0,127}));
    annotation (
      defaultComponentName="dieselGen1",
      Documentation(info="<html>
...
<p>Note: for turbines with gear <tt>w_start</tt> denotes the initial angular velocity at the generator-side!</p>
  ...
  package Partials "Partial models"
    ...
    partial model GenBase_el "Generation base el, synchron machines"
      ...
      parameter SI.Angle alpha_start=system.alpha0
        "initial voltage phase angle" annotation (Dialog(enable=initType ==
              PowerSystems.Types.Init.v_alpha, tab="Initialization"));
      parameter SIpu.Power p_start(unit="1") = 1
        "initial terminal active power" annotation (Dialog(enable=initType ==
              PowerSystems.Types.Init.v_p or initType == PowerSystems.Types.Init.p_q,
            tab="Initialization"));
      parameter SIpu.Power q_start(unit="1") = 1
        "initial terminal reactive power" annotation (Dialog(enable=initType
               == PowerSystems.Types.Init.v_q or initType == PowerSystems.Types.Init.p_q,
            tab="Initialization"));
      ...
      replaceable model Generator = PowerSystems.AC3ph.Machines.Synchron3rd_ee
        "synchron generator" annotation (choices(choice(redeclare model
              Generator = PowerSystems.AC3ph.Machines.Synchron3rd_ee
              "3rd order"), choice(redeclare model Generator =
      ...
      replaceable model Excitation =
          PowerSystems.AC3ph.Machines.Control.Excitation (V_nom=generator.par.V_nom,
            Vf_nom=generator.Vf_nom) "exciter (electric)" annotation (
          choicesAllMatching=true);
      Excitation excitation "exciter (electric)"
        annotation (Placement(transformation(extent={{60,20},{40,40}})));

      replaceable model Governor = PowerSystems.Control.Governors.GovernorConst
        "governor (control)" annotation (choices(choice(redeclare model
              Governor = PowerSystems.Control.Governors.GovernorConst
              "constant"), choice(redeclare model Governor =
                PowerSystems.Control.Governors.Governor1st "1st order")));
      Governor governor "governor (control)"
        annotation (Placement(transformation(extent={{-60,50},{-40,70}})));
      ...
      parameter SI.Time H=10 "inertia cst turb + gen";
    protected
      final parameter SI.AngularVelocity w_nom=2*pi*generator.par.f_nom/
          generator.par.pp "nominal angular velocity";
    ...
    equation
      ...
      if Connections.isRoot(term.theta) then
        term.theta = if system.synRef then {0,system.theta} else {system.theta,
          0};
      end if;
      connect(sender.sendFreq, system.receiveFreq);
      connect(term, generator.term) annotation (Line(points={{100,0},{80,0},{80,
              0},{60,0}}, color={0,130,175}));
      connect(exciter.fieldVoltage, excitation.fieldVoltage)
        annotation (Line(points={{44,50},{44,40}}, color={0,0,127}));
      connect(excitation.termVoltage, exciter.termVoltage)
        annotation (Line(points={{56,40},{56,50}}, color={0,0,127}));
      connect(setpts[1], governor.setptSpeed) annotation (Line(points={{-100,
              6.66667},{-88,6.66667},{-88,64},{-60,64}}, color={0,0,127}));
      connect(setpts[2], governor.setptPower) annotation (Line(points={{-100,-4.44089e-016},
              {-80,-4.44089e-016},{-80,56},{-60,56}}, color={0,0,127}));
      connect(setpts[3], exciter.setptVoltage) annotation (Line(points={{-100,-6.66667},
              {-70,-6.66667},{-70,80},{70,80},{70,60},{60,60}}, color={0,0,127}));
      connect(generator.term, excitation.term) annotation (Line(points={{60,0},
              {80,0},{80,30},{60,30}}, color={0,120,120}));
      connect(excitation.field, generator.field) annotation (Line(points={{60,
              26},{76,26},{76,-4},{60,-4}}, color={0,0,255}));
...
Constant setpoint values can be obtained at (steady-state) initialisation when using Control.CstSetpointsGen.</p>
    ...
    partial model GenBase_ctrl "Generation base pm, synchronous machines"
      ...
      replaceable model Rotor = PowerSystems.Mechanics.Rotational.Rotor (w(
              start=w_start)) "rotor generator" annotation (choicesAllMatching=
            true);
