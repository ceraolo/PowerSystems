...
Consumes the desired active and reactive power at <b>nominal</b> voltage.</p>
      ...
      Diagram(coordinateSystem(
          preserveAspectRatio=false,
          extent={{-100,-100},{100,100}},
          grid={2,2}), graphics={Line(points={{-70,0},{-4,0}}),Rectangle(
              extent={{-4,21},{-2,11}},
              lineColor={0,0,255},
              fillColor={0,0,255},
              fillPattern=FillPattern.Solid),Rectangle(
              ...
              lineColor={0,0,255},
              fillColor={0,0,255},
              fillPattern=FillPattern.Solid),Line(points={{-70,16},{-4,16}}),
            Line(points={{-70,-16},{-4,-16}}),Line(points={{4,16},{30,16}}),
            Line(points={{4,0},{30,0}}),Line(points={{4,-16},{30,-16}}),
            Rectangle(
              extent={{-20,10},{-10,6}},
              lineColor={0,0,255},
              lineThickness=0.5,
...
Consumes the desired active and reactive power at <b>nominal</b> voltage.</p>
  ...
  equation
    pq_fV = diagonal({1,1} + aV*(sqrt(v2_pu) - 1) + aw*(sum(omega) - system.omega_nom))
      *pq;
...
Consumes the desired active and reactive power at steady state and <b>nominal</b> voltage.</p>
  ...
  package Partials "Partial models"
    ...
    partial model IndLoadBase "Inductive load base, 3-phase dq0"
      extends LoadBase(v(start={v_start[1],v_start[2],0}), i(start={i_start[1],
              i_start[2],0}));
    ...
    protected
      ...
      final parameter PS.Voltage[2] v_start={cos(system.alpha0),sin(system.alpha0)}
          *V_nom;
      final parameter PS.Current[2] i_start=[Z_start[1], Z_start[2]; -Z_start[2],
          Z_start[1]]*v_start/(Z_start*Z_start);
...
"), Icon(coordinateSystem(
    ...
    protected
      ...
      final parameter PS.Voltage[2] v_start={cos(system.alpha0),sin(system.alpha0)}
          *V_nom;
      final parameter PS.Current[2] i_start=[Y_start[1], -Y_start[2]; Y_start[2],
          Y_start[1]]*v_start;
