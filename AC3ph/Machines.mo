...
package Machines "AC machines, electric part "
  ...
  initial equation
    phi_el = phi_el_start;
    annotation (defaultComponentName="asynchron", Documentation(info="<html>
...
<p>More information see Partials.AsynchronBase.</p>
  ...
  model AsynchronY_D "Asynchronous machine Y-Delta, cage-rotor, 3-phase dq0"
    extends Partials.AsynchronBase(redeclare model Topology_p =
          PowerSystems.AC3ph.Ports.Topology.Y_Delta (control=YDcontrol)
        "Y-Delta", redeclare replaceable record Data =
          PowerSystems.AC3ph.Machines.Parameters.Asynchron_cage);
...
<p>More information see Partials.AsynchronBase.</p>
  ...
  model Asynchron_ctrl
    ...
    parameter PS.Current[n_r] i_d_start=zeros(n_r)
      "start value of current d_axis" annotation (Dialog(tab="Initialization"));
    parameter PS.Current[n_r] i_q_start=zeros(n_r)
      "start value of current q_axis" annotation (Dialog(tab="Initialization"));
  ...
  equation
    der(phiRotorflux) = w_el - (-diagonal(R_r)*i_rd*psi_rq + diagonal(R_r)*i_rq
      *psi_rd)/(psi_rd*psi_rd + psi_rq*psi_rq + eps);
    alpha_i = atan2(i[2], i[1]);
    alpha_psi = atan2(R_m*psi_rq, R_m*psi_rd);
    i_meas = {cos(alpha_i - alpha_psi),sin(alpha_i - alpha_psi)}*sqrt(i[1:2]*i[
      1:2])/I_nom;
    i_dq = i_act*I_nom;
    (sum(omega) - w_el)*(-L_m*i_dq[2] - L_r*i_q) + diagonal(R_r)*i_d = zeros(
      n_r);
    (sum(omega) - w_el)*(L_m*i_dq[1] + L_r*i_d) + diagonal(R_r)*i_q = zeros(n_r);
    v_dq = sum(omega)*{-(c.L_s[2]*i_dq[2] + L_m*i_q),c.L_s[1]*i_dq[1] + L_m*i_d}
       + c.R_s*i_dq;
    vPhasor = {sqrt(v_dq*v_dq)/par.V_nom,atan2(v_dq[2], v_dq[1]) + atan2(sin(
      alpha_psi - term.theta[1]), cos(alpha_psi - term.theta[1]))};
    annotation (
...
"),
  ...
  model Asynchron_dfig "Doubly fed induction generator, 3-phase dq0"
    extends AC3ph.Machines.Partials.AsynchronBase(v_rd=term_r.v[1]*ones(n_r),
        v_rq=term_r.v[2]*ones(n_r));
    ...
    AC3ph.Ports.ACdq0_p term_r "positive terminal" annotation (Placement(
...
"),
  ...
  model Synchron3rd_ee
    ...
    final parameter PS.Voltage Vf_nom=par.V_nom;
    // to be accessible from 'excitation'.
    output SI.Angle powerAngle(start=0, stateSelect=StateSelect.never)
      "power angle";
    AC1ph_DC.Ports.TwoPin_p field "field winding" annotation (Placement(
          transformation(
          origin={-100,-40},
          extent={{10,10},{-10,-10}},
          rotation=180)));
  ...
  equation
    ...
    powerAngle = noEvent(mod(phi_el - term.theta[2] - atan2(-v[1], v[2]) + pi,
      2*pi)) - pi;
    annotation (defaultComponentName="synchron", Documentation(info="<html>
<p>Equivalent circuit is on <b>diagram layer</b> of parameter record ACdq0.Machines.Parameters.*</p>
...
<p>More information see Partials.Synchron3rdBase.</p>
  ...
  model Synchron_ee "Synchronous machine electrically excited, 3-phase dq0"
    ...
    final parameter PS.Voltage Vf_nom=c.Vf_nom;
    // to be accessible from 'excitation'.
    output SI.Angle powerAngle(start=0, stateSelect=StateSelect.never)
      "power angle";
    AC1ph_DC.Ports.TwoPin_p field "field winding" annotation (Placement(
...
<p>More information see Partials.SynchronBase.</p>
      ...
      Diagram(coordinateSystem(
          ...
          grid={2,2}), graphics={Rectangle(
              ...
              fillPattern=FillPattern.Solid),Line(points={{-50,-44},{-70,-44}},
            color={0,0,255}),Line(points={{30,-44},{40,-44},{40,-36},{-70,-36}},
            color={0,0,255}),Text(
              extent={{30,-39},{90,-49}},
              lineColor={0,0,255},
              textString="field")}));
  ...
  equation
    if par.excite == 1 then
      assert(false,
        "machine-parameter must be excite = 2 (pm) or 3 (reluctance)");
    elseif par.excite == 2 then
      psi_e = c.Psi_pm;
      // = par.psi_pm*(par.V_nom/c.omega_nom)
...
"));
  ...
  model Synchron_pm_ctrl
    ...
    Modelica.Blocks.Interfaces.RealOutput[2] i_meas(final unit="1")
      "measured current {i_d, i_q} pu" annotation (Placement(transformation(
          origin={-60,100},
          extent={{-10,-10},{10,10}},
          rotation=90)));
    Modelica.Blocks.Interfaces.RealInput[2] i_act(final unit="1")
      "actuated current {i_d, i_q} pu" annotation (Placement(transformation(
          origin={60,100},
          extent={{10,-10},{-10,10}},
          rotation=90)));
...
"),
        ...
        Diagram(coordinateSystem(
            ...
            grid={2,2}), graphics={Rectangle(
                  ...
                  lineThickness=0.5,
                  fillColor={255,255,255},
                  fillPattern=FillPattern.Solid),Line(points={{-80,-16},{-50,-16}},
              color={0,0,255}),Line(points={{-80,0},{-50,0}}, color={0,0,255}),
              Line(points={{-80,16},{-50,16}}, color={0,0,255}),Text(
                  extent={{-40,40},{40,30}},
                  lineColor={0,0,255},
                  textString="stator"),Text(
                  extent={{-40,-90},{40,-100}},
                  lineColor={0,0,255},
                  textString="rotor")}));
    end ACmachine;
    ...
    partial model AsynchronBase "Asynchronous machine base, 3-phase dq0"
      extends ACmachine(final pp=par.pp, v(start={cos(system.alpha0),sin(system.alpha0),
              0}*par.V_nom));
    ...
    protected
      ...
      final parameter Coefficients.Asynchron c=
          Utilities.Precalculation.machineAsyn(par, top.scale);
    ...
    equation
      ...
      heat.ports.Q_flow = -{c.R_s*i*i,diagonal(R_r)*i_rd*i_rd + diagonal(R_r)*
        i_rq*i_rq};
      annotation (
        defaultComponentName="asynchron",
        Documentation(info="<html>
  ...
  omega[2] = der(term.theta[2])
        ...
        Diagram(coordinateSystem(
            ...
            grid={2,2}), graphics={Rectangle(
                  ...
                  lineColor={175,175,175},
                  fillColor={175,175,175},
                  fillPattern=FillPattern.Solid),Line(points={{-50,-86},{-60,-86},
              {-60,-40},{-50,-40},{-50,-42}}, color={0,0,255}),Line(points={{30,
              -86},{40,-86},{40,-40},{30,-40},{30,-42}}, color={0,0,255}),Line(
                  points={{30,-42},{30,-86}},
                  color={0,0,255},
                  thickness=0.5),Line(
                  points={{-50,-42},{-50,-86}},
    ...
    protected
      parameter PS.Current[3] i_s_start=cat(
              1,
              inv(Utilities.Transforms.rotation_dq(phi_el_start))*i_start[1:2],
              {i_start[3]}) "start value of stator current dq0 in rotor-system";
...
<pre>
  v_s, i_s:    stator-voltage and -current dq0 in the rotor frame of the machine.
    ...
    partial model Synchron3rdBase "Synchronous machine 3rd base, 3-phase dq0"
      extends SynTransform(final pp=par.pp, v(start={cos(system.alpha0),sin(
              system.alpha0),0}*par.V_nom));
      ...
      final parameter Data par "machine parameters"
        annotation (Placement(transformation(extent={{-60,60},{-40,80}})));
    protected
      final parameter Coefficients.Synchron3rd c=
          Utilities.Precalculation.machineSyn3rd(par, top.scale);
      SI.MagneticFlux[2] psi_s "magnetic flux stator dq";
...
<pre>
  powerAngle:   difference (angle of rotor) - (angle of terminal_voltage)
    ...
    partial model SynchronBase "Synchronous machine base, 3-phase dq0"
      ...
      replaceable record Data = PowerSystems.AC3ph.Machines.Parameters.Synchron
          (f_nom=system.f_nom) "machine parameters" annotation (
    ...
    protected
      ...
      final parameter Coefficients.Synchron c=
          Utilities.Precalculation.machineSyn(par, top.scale);
    ...
    equation
      ...
      tau_el = i_s[1:2]*{-psi_s[2],psi_s[1]};
      heat.ports.Q_flow = -{c.R_s*i_s*i_s,diagonal(R_rd)*i_rd*i_rd + diagonal(
        R_rq)*i_rq*i_rq};
      annotation (
        Documentation(info="<html>
...
<pre>
  powerAngle:   difference (angle of rotor) - (angle of terminal_voltage)
        ...
        Diagram(coordinateSystem(
            ...
            grid={2,2}), graphics={Rectangle(
                  ...
                  lineColor={175,175,175},
                  fillColor={175,175,175},
                  fillPattern=FillPattern.Solid),Line(points={{-50,-60},{-60,-60},
              {-60,-54},{40,-54},{40,-60},{30,-60}}, color={0,0,255}),Line(
              points={{-50,-76},{-60,-76},{-60,-70},{40,-70},{40,-76},{30,-76}},
  ...
  package Control
    ...
    equation
      ...
      annotation (
        defaultComponentName="excitation",
        Documentation(info="<html>
...
"),
    ...
    equation
      ...
      phi_el = delta + atan2(-term.v[1], term.v[2]) + term.theta[2];
      // steady state!
...
"),
        Icon(coordinateSystem(
            ...
            grid={2,2}), graphics={Rectangle(
                  ...
                  fillPattern=FillPattern.Solid),Line(points={{80,0},{-20,0},{
              60,40}}, color={95,95,95})}));
    end PowerAngle;
  ...
  package Parameters "Parameter data for interactive use"
    ...
    record Asynchron3rd "Asynchronous machine 3rd order parameters"
      ...
      SIpu.Resistance r_n "resistance neutral to grd (if Y)"
        annotation (Dialog(enable=not neu_iso));

      annotation (
        defaultComponentName="asyn3rdPar",
...
<p>Equivalent circuit on <b>diagram layer</b>!</p>
        ...
        Diagram(coordinateSystem(
            preserveAspectRatio=false,
            extent={{-100,-100},{100,100}},
            grid={2,2}), graphics={Line(points={{-40,80},{-40,10}}, color={0,0,
              255}),Line(points={{-40,-20},{-40,-90}}, color={0,0,255}),Text(
                  extent={{-66,90},{-52,84}},
                  lineColor={0,0,255},
                  fillColor={255,255,255},
                  fillPattern=FillPattern.Solid,
                  textString="xsig_s"),Text(
                  extent={{-96,90},{-82,84}},
                  lineColor={0,0,255},
                  fillColor={255,255,255},
                  fillPattern=FillPattern.Solid,
                  ...
                  fillColor={255,255,255},
                  fillPattern=FillPattern.Solid,
                  textString="d-axis"),Text(
                  extent={{-100,-60},{-60,-80}},
                  lineColor={0,0,0},
                  fillColor={255,255,255},
                  fillPattern=FillPattern.Solid,
                  textString="q-axis"),Text(
                  extent={{-86,-54},{-46,-60}},
                  lineColor={0,0,255},
                  fillColor={255,255,255},
                  fillPattern=FillPattern.Solid,
                  textString="xm = x - xsig_s"),Text(
                  extent={{-86,46},{-46,40}},
                  lineColor={0,0,255},
                  fillColor={255,255,255},
                  fillPattern=FillPattern.Solid,
                  textString="xm = x - xsig_s"),Rectangle(
                  extent={{-42,60},{-38,30}},
                  lineColor={0,0,255},
                  fillColor={0,0,255},
                  fillPattern=FillPattern.Solid),Rectangle(
                  ...
                  lineColor={0,0,255},
                  fillColor={0,0,255},
                  fillPattern=FillPattern.Solid),Line(points={{-80,-20},{-40,-20}},
              color={0,0,255}),Line(points={{-100,10},{-40,10}}, color={0,0,255}),
              Text(
                  extent={{-66,-10},{-52,-16}},
                  lineColor={0,0,255},
                  fillColor={255,255,255},
                  fillPattern=FillPattern.Solid,
                  textString="xsig_s"),Rectangle(
                  extent={{-70,-18},{-50,-22}},
                  lineColor={0,0,255},
                  fillColor={0,0,255},
                  fillPattern=FillPattern.Solid),Line(points={{-100,-90},{-40,-90}},
              color={0,0,255}),Line(points={{-100,80},{-40,80}}, color={0,0,255}),
              Rectangle(
                  extent={{-100,82},{-80,78}},
                  lineColor={0,0,255},
                  fillColor={255,255,255},
        ...
        Diagram(coordinateSystem(
            ...
            grid={2,2}), graphics={Line(points={{50,80},{50,10}}, color={0,0,
              ...
              Rectangle(
                  ...
                  fillColor={255,255,255},
                  fillPattern=FillPattern.Solid,
                  textString="xsig_r2"),Text(
                  extent={{66,66},{86,60}},
                  lineColor={0,0,255},
                  fillColor={255,255,255},
                  fillPattern=FillPattern.Solid,
                  textString="xsig_r1"),Text(
                  extent={{28,32},{46,26}},
                  lineColor={0,0,255},
                  fillColor={255,255,255},
                  fillPattern=FillPattern.Solid,
                  textString="r_r2"),Text(
                  extent={{68,32},{86,26}},
                  lineColor={0,0,255},
                  fillColor={255,255,255},
                  fillPattern=FillPattern.Solid,
                  textString="r_r1"),Text(
                  extent={{28,-68},{46,-74}},
                  lineColor={0,0,255},
                  fillColor={255,255,255},
                  fillPattern=FillPattern.Solid,
                  ...
                  fillColor={255,255,255},
                  fillPattern=FillPattern.Solid,
                  textString="xsig_r1"),Line(points={{-40,-20},{90,-20},{90,-30}},
              color={0,0,255})}));
    end Asynchron;
...
&nbsp; &nbsp; For pu-input refer to stator base value <tt>R_base</tt>.</p>
    ...
    record Synchron3rd "Synchronous machine 3rd order parameters"
      ...
      annotation (
        defaultComponentName="syn3rdPar",
        defaultComponentPrefixes="parameter",
        Documentation(info="<html>
<p>Equivalent circuit on <b>diagram layer</b>!</p>
<p>This simplified model uses only main but no transient reactances.</p>
        ...
        Diagram(coordinateSystem(
            ...
            grid={2,2}), graphics={Line(points={{-40,80},{-40,10}}, color={0,0,
              255}),Line(points={{-40,-20},{-40,-90}}, color={0,0,255}),Text(
                  ...
                  fillColor={255,255,255},
                  fillPattern=FillPattern.Solid,
                  textString="xm_q = x_q - xsig_s"),Text(
                  extent={{-86,46},{-46,40}},
                  lineColor={0,0,255},
                  fillColor={255,255,255},
                  fillPattern=FillPattern.Solid,
                  textString="xm_d = x_d - xsig_s"),Rectangle(
                  extent={{-42,60},{-38,30}},
                  lineColor={0,0,255},
                  fillColor={0,0,255},
                  fillPattern=FillPattern.Solid),Rectangle(
    ...
    record Synchron "Synchronous machine parameters"
      ...
      SI.Time[n_q] to_q "time constant open-loop q-axis {to_q', to_q'', ..}"
        annotation (Dialog(enable=transDat and not use_xtr));
      Boolean use_if0 "induced field current and phase available?" annotation (
          Dialog(enable=transDat and size(tc_d, 1) > 1 and not pm_exc), choices(
            choice=true "d-axis with xm_d", choice=false "d-axis omitting xm_d"));
      SIpu.Current if0 "induced field current at v_s=Vnom/0deg" annotation (
          Dialog(enable=transDat and size(tc_d, 1) > 1 and use_if0 and not
              pm_exc));
      SI.Angle alpha_if0 "angle(if0) at v_s=Vnom/0deg (sign: i_f behind v_s)"
        annotation (Dialog(enable=transDat and size(tc_d, 1) > 1 and use_if0
               and not pm_exc));
      Real tol "tolerance precalculation" annotation (Dialog(enable=transDat
               and size(tc_d, 1) > 1 and use_if0 and not pm_exc));
...
<pre>
  ...
  Rf = resistance field winding   (in Ohm, true value, not scaled)
        ...
        Diagram(coordinateSystem(
            ...
            grid={2,2}), graphics={Line(points={{10,80},{10,10}}, color={0,0,
              ...
              points={{90,-50},{90,-60}}, color={0,0,255}),Text(
                  ...
                  fillColor={255,255,255},
                  fillPattern=FillPattern.Solid,
                  textString="xsig_rd3"),Text(
                  extent={{-12,32},{6,26}},
                  lineColor={0,0,255},
                  fillColor={255,255,255},
                  fillPattern=FillPattern.Solid,
                  textString="r_rd3"),Text(
                  extent={{-16,90},{-2,84}},
                  lineColor={0,0,255},
                  fillColor={255,255,255},
                  fillPattern=FillPattern.Solid,
                  textString="xm_d3"),Text(
                  extent={{24,90},{38,84}},
                  lineColor={0,0,255},
                  fillColor={255,255,255},
                  fillPattern=FillPattern.Solid,
                  textString="xm_d2"),Ellipse(extent={{64,16},{76,4}},
              lineColor={0,0,255}),Text(
                  extent={{78,6},{92,0}},
                  lineColor={0,0,255},
                  fillColor={255,255,255},
                  fillPattern=FillPattern.Solid,
                  textString="v_f / pm"),Line(points={{-40,80},{90,80},{90,70}},
              color={0,0,255}),Rectangle(
                  extent={{88,70},{92,50}},
                  lineColor={0,0,255},
                  fillColor={0,0,255},
                  ...
                  fillColor={255,255,255},
                  fillPattern=FillPattern.Solid,
                  textString="xsig_rd2"),Text(
                  extent={{66,66},{86,60}},
                  lineColor={0,0,255},
                  fillColor={255,255,255},
                  fillPattern=FillPattern.Solid,
                  textString="xsig_rd1"),Text(
                  extent={{28,32},{46,26}},
                  lineColor={0,0,255},
                  fillColor={255,255,255},
                  fillPattern=FillPattern.Solid,
                  textString="r_rd2"),Text(
                  extent={{68,32},{86,26}},
                  lineColor={0,0,255},
                  fillColor={255,255,255},
                  fillPattern=FillPattern.Solid,
                  textString="r_rd1"),Text(
                  extent={{-12,-68},{6,-74}},
                  lineColor={0,0,255},
                  fillColor={255,255,255},
                  fillPattern=FillPattern.Solid,
                  textString="r_rq3"),Text(
                  extent={{28,-68},{46,-74}},
                  lineColor={0,0,255},
                  fillColor={255,255,255},
                  fillPattern=FillPattern.Solid,
                  textString="r_rq2"),Text(
                  extent={{68,-68},{86,-74}},
                  lineColor={0,0,255},
                  fillColor={255,255,255},
                  fillPattern=FillPattern.Solid,
                  textString="r_rq1"),Text(
                  extent={{-14,-34},{6,-40}},
                  lineColor={0,0,255},
                  fillColor={255,255,255},
                  fillPattern=FillPattern.Solid,
                  textString="xsig_rq3"),Text(
                  extent={{26,-34},{46,-40}},
                  lineColor={0,0,255},
                  fillColor={255,255,255},
                  fillPattern=FillPattern.Solid,
                  textString="xsig_rq2"),Text(
                  extent={{66,-34},{86,-40}},
                  lineColor={0,0,255},
                  fillColor={255,255,255},
                  fillPattern=FillPattern.Solid,
    ...
    record Synchron3rd_ee "Synchronous machine 3rd order parameters"
      ...
      annotation (
        defaultComponentName="syn3rd_elPar",
        defaultComponentPrefixes="parameter",
        Documentation(info="<html>
<p>Equivalent circuit on <b>diagram layer</b>!</p>
<p>This simplified model uses only main but no transient reactances.</p>
    ...
    record Synchron_ee "Synchronous machine parameters"
      ...
      annotation (
        defaultComponentName="syn_elPar",
        defaultComponentPrefixes="parameter",
        Documentation(info="<html>
<p>Equivalent circuit on <b>diagram layer</b>!</p>
<p>Specifying standard transient data both for _d and _q axis:</p>
  ...
  Rf = resistance field winding   (in Ohm, true value, not scaled)
    ...
    record Synchron3rd_pm "Synchronous machine pm 3rd order parameters"
      extends Synchron3rd(
        ...
        r_n=1);
      annotation (
        defaultComponentName="syn3rd_pmPar",
        defaultComponentPrefixes="parameter",
        Documentation(info="<html>
<p>Equivalent circuit on <b>diagram layer</b>!</p>
<p>The relation between 'flux induced by permanent magnet' <tt>Psi_pm [Wb]</tt> and 'magnetisation' <tt>psi_pm [pu]</tt> is given by the following relation;
...
"));
    ...
    record Synchron_pm "Synchronous machine pm parameters"
      ...
      annotation (
        defaultComponentName="syn_pmPar",
        defaultComponentPrefixes="parameter",
        Documentation(info="<html>
<p>Equivalent circuit on <b>diagram layer</b>!</p>
<p>Specifying standard transient data both for _d and _q axis:</p>
  ...
  psi_pm = Psi_pm*omega_nom/V_nom
    ...
    record Synchron3rd_reluctance "Synchronous machine pm 3rd order parameters"
      extends Synchron3rd(
        ...
        r_n=1);
      annotation (
        defaultComponentName="syn_reluctPar",
        defaultComponentPrefixes="parameter",
        Documentation(info="<html>
<p>Equivalent circuit on <b>diagram layer</b>!</p>
</html>"));
...
<p>Records containing parameters of the corresponding components.</p>
  ...
  package Coefficients "Coefficient matrices of machine equations"
    ...
    record Asynchron "Coefficient matrices of asynchronous machine"
      ...
      annotation (defaultComponentPrefixes="final parameter", Documentation(
            info="<html>
</html>"));
    end Asynchron;
