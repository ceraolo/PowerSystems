...
<p>AC connector with vector variables in dq0-representation, negative.</p>
  ...
  partial model Port_f "AC one port 'fault', 3-phase"
    ...
    Ports.ACdq0_p term "fault terminal" annotation (Placement(transformation(
          origin={0,-100},
          extent={{-10,-10},{10,10}},
          rotation=90)));
    annotation (Icon(coordinateSystem(preserveAspectRatio=false, extent={{-100,
  ...
  partial model Port_p_n_f "AC three port, 3-phase"
    ...
    Ports.ACdq0_n term_f "fault terminal" annotation (Placement(transformation(
          origin={0,100},
          extent={{-10,-10},{10,10}},
          rotation=90)));
  ...
  equation
    ...
    annotation (Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-100,
              -100},{100,100}}), graphics={Line(points={{30,-16},{52,-16},{62,0},
            {52,16},{30,16}}, color={0,0,255}),Line(points={{30,0},{70,0}},
...
*/
    ...
    replaceable model Topology_p = Topology.Y constrainedby
      Topology.TopologyBase "Y or Delta topology" annotation (choices(choice(
            redeclare model Topology_p = PowerSystems.AC3ph.Ports.Topology.Y
            "Y"), choice(redeclare model Topology_p =
              PowerSystems.AC3ph.Ports.Topology.Delta "Delta")));
    Topology_p top(
      v_cond=v,
      i_cond=i,
      v_n=v_n) annotation (Placement(transformation(extent={{30,-20},{70,20}})));
...
"));
  ...
  partial model YDport_n "AC one port Y or Delta topology 'positive'"
    ...
    replaceable model Topology_n = Topology.Y constrainedby
      Topology.TopologyBase "Y or Delta topology" annotation (choices(choice(
            redeclare model Topology_p = PowerSystems.AC3ph.Ports.Topology.Y
            "Y"), choice(redeclare model Topology_p =
              PowerSystems.AC3ph.Ports.Topology.Delta "Delta")));
...
"));
  ...
  partial model Y_Dport_p "AC two port, switcheable Y-Delta topology"
    ...
    replaceable model Topology_p = Topology.Y_Delta constrainedby
      Topology.TopologyBase "Y-Delta switcheable" annotation (choices(choice(
            redeclare model Topology_p =
              PowerSystems.AC3ph.Ports.Topology.Y_Delta "Y_Delta"), choice(
            redeclare model Topology_p =
              PowerSystems.AC3ph.Ports.Topology.Y_DeltaRegular "Y_DeltaRegular")));
    Topology_p top(
      v_cond=v,
      i_cond=i,
      control=YDcontrol) "Y-Delta switcheable"
      annotation (Placement(transformation(extent={{30,-20},{70,20}})));
    PS.Voltage[3] v "voltage conductor";
    PS.Current[3] i "current conductor";
...
"));
  ...
  partial model YDportTrafo_p_n
    ...
    replaceable model Topology_p = Topology.Y constrainedby
      Topology.TopologyBase "p: Y, Delta or PAR topology" annotation (choices(
        choice(redeclare model Topology_p = PowerSystems.AC3ph.Ports.Topology.Y
            "Y"),
    ...
    Topology_p top_p(
      ...
      i_cond=i1,
      v_n=v_n1,
      w=w1) "p: Y, Delta or PAR topology"
      annotation (Placement(transformation(extent={{-80,-20},{-40,20}})));

    replaceable model Topology_n = Topology.Y constrainedby
      Topology.TopologyBase "n: Y, Delta or PAR topology" annotation (choices(
        choice(redeclare model Topology_n = PowerSystems.AC3ph.Ports.Topology.Y
            "Y"),
    ...
    Topology_n top_n(
      ...
      i_cond=i2/w2_nom,
      v_n=v_n2,
      w=w2) "n: Y, Delta or PAR topology"
      annotation (Placement(transformation(extent={{80,-20},{40,20}})));
  ...
  protected
    ...
    final parameter Integer n_n2=top_n.n_n annotation (Evaluate=true);
...
"));
  ...
  partial model YDportTrafo_p_n_n
    ...
    replaceable model Topology_p = Topology.Y constrainedby
      Topology.TopologyBase "p: Y or Delta topology" annotation (choices(choice(
            redeclare model Topology_p = PowerSystems.AC3ph.Ports.Topology.Y
            "Y"), choice(redeclare model Topology_p =
              PowerSystems.AC3ph.Ports.Topology.Delta "Delta")));
    Topology_p top_p(
      ...
      i_cond=i1,
      v_n=v_n1,
      w=w1) "p: Y or Delta topology"
      annotation (Placement(transformation(extent={{-80,-20},{-40,20}})));

    replaceable model Topology_na = Topology.Y constrainedby
      Topology.TopologyBase "na: Y or Delta topology" annotation (choices(
          choice(redeclare model Topology_na =
              PowerSystems.AC3ph.Ports.Topology.Y "Y"), choice(redeclare model
            Topology_na = PowerSystems.AC3ph.Ports.Topology.Delta "Delta")));
    Topology_na top_na(
      v_cond=v2a*w2a_nom,
      i_cond=i2a/w2a_nom,
      v_n=v_n2a,
      w=w2a) "na: Y or Delta topology"
      annotation (Placement(transformation(extent={{80,20},{40,60}})));

    replaceable model Topology_nb = Topology.Y constrainedby
      Topology.TopologyBase "nb: Y or Delta topology" annotation (choices(
          choice(redeclare model Topology_nb =
              PowerSystems.AC3ph.Ports.Topology.Y "Y"), choice(redeclare model
            Topology_nb = PowerSystems.AC3ph.Ports.Topology.Delta "Delta")));
    Topology_nb top_nb(
      v_cond=v2b*w2b_nom,
      i_cond=i2b/w2b_nom,
      v_n=v_n2b,
      w=w2b) "nb: Y or Delta topology"
  ...
  protected
    ...
    final parameter Integer n_n1=top_p.n_n annotation (Evaluate=true);
    final parameter Integer n_n2a=top_na.n_n annotation (Evaluate=true);
    final parameter Integer n_n2b=top_nb.n_n annotation (Evaluate=true);
    Real w1 "1: voltage ratio to nominal for topology";
...
<p>Note: parameter sh (phase shift) not used.</p>
        ...
        Icon(coordinateSystem(
            ...
            grid={2,2}), graphics={Line(
                  points={{-60,0},{60,0}},
                  color={255,0,0},
                  thickness=0.5),Line(points={{60,0},{100,0}}, color={0,0,255}),
              Line(
                  points={{-60,80},{10,80},{60,0},{10,-80},{-60,-80}},
                  color={255,0,0},
                  thickness=0.5),Line(points={{-100,80},{-60,80}}, color={0,0,
              255}),Line(points={{-100,0},{-60,0}}, color={0,0,255}),Line(
              points={{-100,-80},{-60,-80}}, color={0,0,255})}),
        Diagram(coordinateSystem(
            preserveAspectRatio=false,
            extent={{-100,-100},{100,100}},
            grid={2,2}), graphics={Line(points={{-88,80},{-60,80}}, color={0,0,
              255}),Line(points={{-90,0},{-60,0}}, color={0,0,255}),Line(points=
               {{-90,-80},{-60,-80}}, color={0,0,255}),Line(
                  ...
                  points={{-60,0},{60,0}},
                  color={255,0,0},
                  thickness=0.5),Line(points={{60,0},{90,0}}, color={0,0,255})}));
    end Y;
    ...
    model Delta "Delta transform"
      ...
      parameter Integer shift(
        min=0,
        max=11) = 1 "(0, 1 .. 11)*30deg phase shift" annotation (Evaluate=true);
...
"),     Icon(coordinateSystem(
            ...
            grid={2,2}), graphics={Line(points={{-100,80},{80,80}}, color={0,0,
              255}),Line(points={{-100,0},{-60,0}}, color={0,0,255}),Line(
              points={{-100,-80},{80,-80}}, color={0,0,255}),Polygon(
  ...
  "),   Icon(coordinateSystem(
            ...
            grid={2,2}), graphics={Line(points={{60,-5},{100,-5}}, color={0,0,
              255}),Line(
                  ...
                  points={{-60,-70},{80,-70}},
                  color={0,0,255},
                  pattern=LinePattern.Dot),Line(points={{-100,75},{-70,75}},
              color={0,0,255}),Polygon(
                  ...
                  lineColor={255,0,255},
                  fillColor={255,0,255},
                  fillPattern=FillPattern.Solid),Line(points={{-100,0},{-70,0}},
              color={0,0,255}),Line(points={{-100,-75},{-70,-75}}, color={0,0,
              255})}),
        Diagram(coordinateSystem(
            preserveAspectRatio=false,
            extent={{-100,-100},{100,100}},
            grid={2,2}), graphics={Line(points={{-70,75},{-60,70}}, color={255,
              0,255}),Line(points={{-70,-75},{-60,-80}}, color={255,0,255}),
              Line(
                  points={{-70,75},{-60,80}},
                  color={255,0,255},
                  pattern=LinePattern.Dot),Line(
                  points={{-70,-75},{-60,-70}},
                  color={255,0,255},
                  pattern=LinePattern.Dot),Line(points={{-70,0},{-60,-5}},
              ...
              -75}}, color={0,0,255}),Line(
                  ...
                  points={{-60,80},{80,80}},
                  color={0,0,255},
                  pattern=LinePattern.Dot),Line(points={{-80,-20},{-60,-20}},
              color={255,0,255}),Line(
                  points={{-80,10},{-60,10}},
                  color={255,0,255},
                  pattern=LinePattern.Dot)}));
...
<p>More info see Topology.Y and Topology.Delta.</p>
  ...
  "),   Icon(coordinateSystem(
    ...
    model PAR "Phase angle regulating (quadrature booster)"
      extends PowerSystems.AC3ph.Ports.Topology.TopologyBase(final scale=1,
          final n_n=1);

      SI.Angle alpha=atan(w - 1) "phase shift";

    protected
      Real[2, 2] Rot=[1, -w + 1; w - 1, 1]
        "Transforms.rotation_dq(alpha)/cos(alpha)";
...
<p>Note: parameter sh (phase shift) not used.</p>
        ...
        Icon(coordinateSystem(
            ...
            grid={2,2}), graphics={Line(
                  ...
                  color={255,0,0},
                  thickness=0.5),Line(points={{-100,80},{100,80}}, color={0,0,
              255}),Line(points={{60,-80},{60,10}}, color={0,0,255}),Line(
              points={{-100,50},{100,50}}, color={0,0,255}),Line(points={{-100,
              20},{100,20}}, color={0,0,255})}),
        Diagram(coordinateSystem(
            preserveAspectRatio=false,
            extent={{-100,-100},{100,100}},
            grid={2,2}), graphics={Line(points={{60,-80},{60,80}}, color={0,0,
              255}),Ellipse(
                  extent={{58,82},{62,78}},
                  lineColor={0,0,255},
                  fillColor={0,0,255},
                  ...
                  lineColor={0,0,255},
                  fillColor={0,0,255},
                  fillPattern=FillPattern.Solid),Line(points={{-80,0},{60,60}},
              color={0,0,255}),Ellipse(
                  ...
                  fillColor={0,0,255},
                  fillPattern=FillPattern.Solid,
                  textString="v_cond"),Text(
                  extent={{-20,48},{20,40}},
                  lineColor={0,0,255},
                  fillColor={0,0,255},
                  fillPattern=FillPattern.Solid,
                  textString="v_term"),Text(
                  extent={{-60,10},{-40,2}},
                  lineColor={0,0,255},
                  fillColor={0,0,255},
                  fillPattern=FillPattern.Solid,
                  ...
                  fillColor={0,0,255},
                  fillPattern=FillPattern.Solid,
                  textString="w = dv"),Line(points={{-80,0},{60,0}}, color={0,0,
              255})}));
    end PAR;
