...
"), experiment(StopTime=1));
  ...
  model GenOrder3and7 "Generator-models of different order"
    ...
    PowerSystems.AC3ph.Sources.InfBus infBus2(V_nom=26e3)
      annotation (Placement(transformation(extent={{80,-60},{60,-40}})));
    PowerSystems.Control.Relays.SwitchRelay relayGrd(ini_state=false, t_switch=
          {0.1,0.3})
  ...
  equation
    ...
    connect(turboGen1.term, line1.term_p)
      annotation (Line(points={{-60,50},{20,50}}, color={0,110,110}));
    connect(line1.term_n, infBus1.term)
      annotation (Line(points={{40,50},{60,50}}, color={0,110,110}));
    ...
    connect(line2.term_n, infBus2.term)
      annotation (Line(points={{40,-50},{60,-50}}, color={0,110,110}));
    connect(turboGen2.term, switchGrd2.term_p) annotation (Line(points={{-60,-50},
            {-40,-50},{-40,-80},{-20,-80}}, color={0,110,110}));
    connect(switchGrd2.term_n, grd2f.term)
      annotation (Line(points={{0,-80},{20,-80}}, color={0,110,110}));
    connect(infBus1.neutral, grd1.term)
      annotation (Line(points={{80,50},{80,50}}, color={0,0,255}));
    connect(infBus2.neutral, grd2.term)
      annotation (Line(points={{80,-50},{80,-50}}, color={0,0,255}));
    connect(turboGen1.heat, bdCond1.heat)
      annotation (Line(points={{-70,60},{-70,60}}, color={176,0,0}));
...
"), experiment(StopTime=30));
  ...
  equation
    ...
    connect(sensor.term_n, line.term_p)
      annotation (Line(points={{10,10},{30,10}}, color={0,110,110}));
    connect(line.term_n, infBus.term)
      annotation (Line(points={{50,10},{70,10}}, color={0,110,110}));
    connect(infBus.neutral, grd.term)
      annotation (Line(points={{90,10},{90,10}}, color={0,0,255}));
...
public
  model LocalGeneration
    ...
    PowerSystems.AC3ph.Generation.PMgenerator PMgen(
      redeclare model Generator =
          PowerSystems.AC3ph.Machines.Synchron3rd_pm_ctrl (redeclare record
            Data =
    ...
    PowerSystems.AC1ph_DC.Impedances.CapacitorSym capSym(
      G=1e-4,
      V_start=vDC_nom,
      C=0.020) annotation (Placement(transformation(extent={{0,-60},{20,-40}})));
    PowerSystems.AC3ph.Nodes.GroundOne grd annotation (Placement(transformation(
          origin={10,-80},
          extent={{-10,10},{10,-10}},
          rotation=270)));
  ...
  equation
    connect(PMgen.heat, bdCond.heat)
      annotation (Line(points={{-30,-40},{-30,-40}}, color={176,0,0}));
    ...
    connect(PI_vDC.y, gain.u)
      annotation (Line(points={{-49,30},{-42,30}}, color={0,0,127}));
    connect(gain.y, PMgen.i_act[2]) annotation (Line(points={{-19,30},{-10,30},
            {-10,10},{-36,10},{-36,-40.5}}, color={0,0,127}));
