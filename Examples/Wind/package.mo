...
model WindTurbine_DFIG
  ...
  Modelica.Blocks.Sources.RealExpression vPhasor_set[2](y={0.5 - min(windSpeed.y,
        15)/20,0.25*windSpeed.y/15})
...
model WindTurbine_PSGR
  ...
  PowerSystems.Common.Thermal.BdCondV bdCond(m=2) annotation (Placement(
        transformation(extent={{-70,-24},{-50,-4}}, rotation=0)));
  PowerSystems.AC3ph.Inverters.RectifierAverage rectifier1(redeclare record
      Data = Semiconductors.Ideal.SCparameter)
...
model WindFarm "Multiple wind turbines connected together"
  ...
  PowerSystems.AC3ph.Lines.PIline line1(
    ne=1,
    len=100,
    redeclare record Data = PowerSystems.AC3ph.Lines.Parameters.Line (V_nom=
            15e3, S_nom=3e6)) annotation (Placement(transformation(
        extent={{-6,-6},{6,6}},
        rotation=0,
        origin={-10,70})));
...
1998"
    ...
    Modelica.Mechanics.Rotational.Interfaces.Flange_b flange(tau(start=-P_nom/
            10))
