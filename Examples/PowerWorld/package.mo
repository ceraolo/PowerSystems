...
model PowerWorld "Interoperation of wind power and thermal power"
  ...
  PowerSystems.Generic.Impedance lineWind(R=1, L=1/314) annotation (Placement(
        transformation(
        extent={{-6,-6},{6,6}},
        rotation=-90,
        origin={10,10})));
  PowerSystems.Generic.Impedance lineHydro(R=1, L=1/314) annotation (Placement(
        transformation(
        extent={{-6,-6},{6,6}},
        rotation=-90,
        origin={40,-10})));
  Modelica.Blocks.Sources.RealExpression frequency(y=system.omega/2/pi)
    "Average frequency" annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=-90,
        origin={-80,-20})));
  inner System system(dynType=PowerSystems.Types.Dynamics.FixedInitial, fType=
        PowerSystems.Types.SystemFrequency.Average)
    annotation (Placement(transformation(extent={{-100,80},{-80,100}})));
  equation
  connect(powerPlant.terminal, trafoPlant.terminal_p) annotation (Line(
      points={{-40,0},{-36,0}},
      color={0,120,120},
      smooth=Smooth.None));
  ...
  annotation (
    ...
    __Dymola_Commands(
      file(ensureSimulated=true) =
        "Examples/PowerWorld/Resources/plot summary.mos" "plot summary",
      file(ensureSimulated=true) =
        "Examples/PowerWorld/Resources/plot powerPlant.mos" "plot powerPlant",
      file(ensureSimulated=true) =
        "Examples/PowerWorld/Resources/plot hydroPlant.mos" "plot hydroPlant"),
...
package Components "Aggregated components for PowerWorld example"
  ...
  model PowerPlant "Thermal Power Plant with primary and secondary control"
    extends PowerSystems.Generic.Ports.PartialSource(final potentialReference=
          true);
    ...
    Modelica.Blocks.Continuous.FirstOrder evaporator(
      T=60,
      y_start=490,
      initType=Modelica.Blocks.Types.Init.InitialState) annotation (Placement(
          ...
          rotation=90,
          origin={-90,10})));
    Modelica.Blocks.Continuous.FirstOrder superheater1(
      T=60,
      y_start=490,
      initType=Modelica.Blocks.Types.Init.InitialState) annotation (Placement(
          transformation(
          ...
          rotation=90,
          origin={-90,50})));
    Modelica.Blocks.Continuous.FirstOrder superheater2(
      T=60,
      y_start=490,
      initType=Modelica.Blocks.Types.Init.InitialState) annotation (Placement(
          transformation(
    ...
    Modelica.Blocks.Math.Add accumulation(k2=1e-2, k1=if Modakond then 5e-4
           else 0)
      annotation (Placement(transformation(extent={{24,-66},{36,-54}})));
    Modelica.Blocks.Sources.Constant zero(k=0)
      annotation (Placement(transformation(extent={{16,-66},{4,-54}})));
    Modelica.Blocks.Continuous.FirstOrder throttleDynamics(
      T=120,
      k=-300/800,
      initType=Modelica.Blocks.Types.Init.SteadyState)
    ...
    Modelica.Blocks.Sources.Constant throttleReserve(k=if Modakond then 0 else
          5) annotation (Placement(transformation(extent={{-12,88},{0,100}})));
  ...
  public
    ...
    connect(rotor.flange_b, angularVelocity.flange) annotation (Line(
        points={{50,-10},{54,-10},{54,10},{46,10}},
        color={0,0,0},
        smooth=Smooth.None));
    connect(primaryControl.y, loadControl.u1) annotation (Line(
        points={{-1,30},{-40,30},{-40,-4},{-32,-4}},
        color={0,0,127},
        smooth=Smooth.None));
    connect(evaporator.y, superheater1.u) annotation (Line(
        points={{-90,21},{-90,38}},
        color={0,0,127},
        smooth=Smooth.None));
    connect(superheater1.y, superheater2.u) annotation (Line(
        points={{-90,61},{-90,78}},
        color={0,0,127},
        smooth=Smooth.None));
    connect(superheater2.y, loadControl.u2) annotation (Line(
        points={{-90,101},{-90,110},{-60,110},{-60,-16},{-32,-16}},
        color={0,0,127},
        smooth=Smooth.None));
    connect(loadControl.y, turbine.tau) annotation (Line(
        points={{-9,-10},{-2,-10}},
        color={0,0,127},
        smooth=Smooth.None));
    ...
    connect(rotor.flange_b, generator.flange) annotation (Line(
        ...
        color={0,0,0},
        smooth=Smooth.None));
    connect(fuel.y, evaporator.u) annotation (Line(
        points={{-90,-19},{-90,-2}},
        color={0,0,127},
        smooth=Smooth.None));
    ...
    connect(loadControl.u1, add.u2) annotation (Line(
        points={{-32,-4},{-40,-4},{-40,-90},{-104,-90},{-104,-83.6},{-97.2,-83.6}},

        color={0,0,127},
        smooth=Smooth.None));
    connect(accumulation.y, hotwellLevel.u) annotation (Line(
    ...
    annotation (Icon(graphics={Polygon(
                ...
                fillColor={189,189,126}),Rectangle(
                extent={{-44,-100},{80,-28}},
                lineColor={0,0,255},
                pattern=LinePattern.None,
                fillColor={0,127,127},
            ...
            preserveAspectRatio=true, extent={{-120,-100},{100,120}})), Diagram(
  ...
  model HydroPlant
    ...
    Modelica.Blocks.Continuous.Integrator reservoirLevel(
      y_start=10,
      k=-1/5e4,
      y(unit="m"))
    ...
    Modelica.Blocks.Math.Gain powerControl(k=1e6/50/2/pi)
      annotation (Placement(transformation(extent={{-36,-6},{-24,6}})));
    Modelica.Blocks.Continuous.TransferFunction controlDynamics(
      a={1,1},
      b={1},
  ...
  public
    ...
    connect(reservoirTurbine.flange, rotor.flange_a) annotation (Line(
        points={{10,0},{20,0}},
        color={0,0,0},
        smooth=Smooth.None));
    connect(rotor.flange_b, generator.flange) annotation (Line(
        ...
        color={0,0,0},
        smooth=Smooth.None));
    connect(powerControl.y, reservoirTurbine.tau) annotation (Line(
        points={{-23.4,0},{-12,0}},
        color={0,0,127},
        smooth=Smooth.None));
    ...
    annotation (Icon(coordinateSystem(preserveAspectRatio=true, extent={{-100,-100},
              {100,100}}), graphics={Text(
                ...
                points={{-18,-18},{16,-56},{36,-34},{76,-36},{18,24},{-104,20},
              {-18,-18}},
                lineColor={0,0,255},
  ...
  model WindFarm
    extends PowerSystems.Generic.Ports.PartialSource(final potentialReference=
          false, final definiteReference=false);
    ...
    PowerSystems.Generic.PrescribedPowerSource mills(redeclare package
        PhaseSystem = PhaseSystem, definiteReference=definiteReference)
  ...
  model LoadDispatcher
    ...
    Modelica.Blocks.Sources.Constant controlHydro(k=200)
      annotation (Placement(transformation(extent={{80,0},{60,20}})));
    Modelica.Blocks.Interfaces.RealInput frequency(unit="Hz") annotation (
        Placement(transformation(
          extent={{-10,-10},{10,10}},
          rotation=-90,
          origin={0,70})));
    Modelica.Blocks.Continuous.LimPID secondaryControl(
      k=60/0.4,
    ...
    Modelica.Blocks.Math.Add3 plantSchedule(
      k2=-1,
      k3=-1,
      y(unit="MW"))
    ...
    Modelica.Blocks.Math.Gain distributionLoss(k=1) annotation (Placement(
          transformation(extent={{-10,-10},{10,10}}, origin={-16,16})));
  protected
    outer System system;
    equation
  ...
  model City
    ...
    annotation (Icon(coordinateSystem(preserveAspectRatio=false, extent={{-100,
              -100},{100,100}}), graphics={Polygon(
                points={{-100,-100},{-100,100},{-66,100},{-66,0},{-40,0},{-40,
              40},{0,40},{0,0},{40,0},{40,60},{80,60},{80,-20},{100,-20},{100,-100},
              {-100,-100}},
                lineColor={0,0,0},
                smooth=Smooth.None,
                fillColor={0,0,0},
...
package Test "Component tests for PowerWorld example"
  ...
  model PowerPlantTest1 "Test primary control"
    ...
    connect(powerPlant.terminal, prescribedLoad.terminal) annotation (Line(
        ...
        color={0,0,0},
        smooth=Smooth.None));
    connect(secondary.y, powerPlant.plantDispatch[2]) annotation (Line(
        points={{-59,10},{-40,10},{-40,4},{-20,4}},
        color={0,0,127},
        smooth=Smooth.None));
    connect(schedule.y, powerPlant.plantDispatch[1]) annotation (Line(
  ...
  model PowerPlantTest3 "Test connection to a large net"
    ...
    Modelica.Blocks.Sources.Ramp secondary(
      ...
      duration=0)
      annotation (Placement(transformation(extent={{-80,0},{-60,20}})));
    PowerSystems.Generic.Impedance line(R=1, L=1/314)
      annotation (Placement(transformation(extent={{20,0},{40,20}})));
    Modelica.Blocks.Sources.Constant primary(k=40)
      annotation (Placement(transformation(extent={{-80,40},{-60,60}})));
    inner PowerSystems.System system
  ...
  model HydroPlantTest1 "Test primary control"
    ...
    inner PowerSystems.System system
      annotation (Placement(transformation(extent={{-100,80},{-80,100}})));
    equation
    connect(schedule.y, hydroPlant.hydroDispatch[1]) annotation (Line(
    ...
    connect(hydroPlant.terminal, prescribedLoad.terminal) annotation (Line(
        ...
        color={0,0,0},
        smooth=Smooth.None));
    connect(primary.y, hydroPlant.hydroDispatch[3]) annotation (Line(
  ...
  model WindFarmLoadTest "WindFarm connected to a load"
    ...
    PowerSystems.Generic.Impedance load(R=30, L=10/314)
      annotation (Placement(transformation(extent={{-20,0},{0,20}})));
  ...
  model WindFarmHVDCTest "WindFarm connected to a large net via HVDC"
    ...
    PowerSystems.Generic.FixedVoltageSource largeGrid
      annotation (Placement(transformation(extent={{40,0},{20,20}})));
    PowerSystems.Generic.Inverter inverter
      annotation (Placement(transformation(extent={{-20,0},{0,20}})));
    inner PowerSystems.System system
      annotation (Placement(transformation(extent={{-100,80},{-80,100}})));
    equation
...
constant String PowerWorldResources=Modelica.Utilities.Files.loadResource(
    "modelica://PowerSystems.Examples.PowerWorld/Resources/");
