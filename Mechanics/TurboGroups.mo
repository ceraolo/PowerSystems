...
package TurboGroups "Turbines including generator-rotor"
  ...
  model FixedSpeedTG "Fixed speed turbo-generator rotor"
    ...
    Modelica.Blocks.Interfaces.RealInput power(final unit="1")
      "turbine power pu" annotation (Placement(transformation(
          origin={60,100},
          extent={{-10,-10},{10,10}},
          rotation=270)));
    Modelica.Blocks.Interfaces.RealOutput speed(final unit="1")
  ...
  equation
    ...
    speed = w/w_nom;
    annotation (
      defaultComponentName="rotor1mass",
...
"),   Icon(coordinateSystem(
  ...
  equation
    delta = {turbine2.flange_a.phi - turbine1.flange_b.phi,turbine3.flange_a.phi
       - turbine2.flange_b.phi,turbine4.flange_a.phi - turbine3.flange_b.phi,
      genRotor.flange_a.phi - turbine4.flange_b.phi};
...
"),   Icon(coordinateSystem(
  ...
  equation
    ...
    connect(blades[1], turbine.rotor)
      annotation (Line(points={{-100,60},{-80,60},{-80,6}}, color={0,0,0}));
    connect(blades[2], compressor.rotor)
      annotation (Line(points={{-100,60},{-50,60},{-50,6}}, color={0,0,0}));
    connect(airgap, genRotor.rotor)
      annotation (Line(points={{100,60},{80,60},{80,6}}, color={0,0,0}));
    annotation (
      defaultComponentName="GTgrp",
      Documentation(info="<html>
...
Therefore phi and w represent the mechanical angle and angular velocity.
  ...
  equation
    ...
    connect(shaft.flange_b, genRotor.flange_a)
      annotation (Line(points={{5,0},{5,0}}, color={0,0,0}));
    connect(blades[1], turbine.rotor)
      annotation (Line(points={{-100,60},{-15,60},{-15,6}}, color={0,0,0}));
    connect(airgap, genRotor.rotor)
      annotation (Line(points={{100,60},{15,60},{15,6}}, color={0,0,0}));
    annotation (
      defaultComponentName="hydroGrp",
      Documentation(info="<html>
...
"),   Icon(coordinateSystem(
  ...
  model Diesel "Diesel with generator-rotor"
    extends Partials.TurboBase3(final n=1);
    // annotation 0;
  ...
  equation
    ...
    connect(airgap, genRotor.rotor)
      annotation (Line(points={{100,60},{15,60},{15,6}}, color={0,0,0}));
    annotation (
      defaultComponentName="dieselGrp",
      Documentation(info="<html>
...
"),   Icon(coordinateSystem(
  ...
  model PcontrolTorque "Turbine torque from power control"
    ...
    parameter SI.AngularVelocity w_nom=314.159265358979323846
      "nom r.p.m. turbine" annotation (Evaluate=true, Dialog(group="Nominal"));
    ...
    Modelica.Blocks.Interfaces.RealInput power(final unit="1") "power pu"
    ...
    Modelica.Blocks.Interfaces.RealOutput speed(final unit="1")
      "angular velocity pu" annotation (Placement(transformation(
          ...
          extent={{-10,-10},{10,10}},
          rotation=90)));
    Interfaces.Rotation_n[n] blades "to turbine model" annotation (Placement(
          transformation(
          origin={100,60},
          extent={{10,10},{-10,-10}},
          rotation=180)));
...
"),   Icon(coordinateSystem(
  ...
  model WindTabTorque "Turbine torque, table {speed, torque pu}"
    ...
    Interfaces.Rotation_n blades "to turbine model" annotation (Placement(
          transformation(
          origin={100,60},
          extent={{10,10},{-10,-10}},
          rotation=180)));
  ...
  package Parameters "Parameter data for interactive use"
    ...
    record SteamTurboGroup "Steam turbo-group parameters"
      ...
      parameter SI.AngularVelocity w_nom=314.159265358979323846
        "nom r.p.m. turbines"
        annotation (Evaluate=true,Dialog(group="Nominal"));
      parameter SI.Power[:] P_nom={500e6,250e6,250e6,250e6}
        "nom power turbines" annotation (Evaluate=true, Dialog(group="Nominal"));
      ...
      parameter SI.Stiffness[size(J_turb, 1) + size(J_aux, 1)] stiff={250,350,
          750,750,750,250}*1e6 "stiffness shafts";
