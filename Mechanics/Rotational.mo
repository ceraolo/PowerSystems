  ...
  equation
    ...
    annotation (
      ...
      Diagram(graphics={Text(
              extent={{-50,10},{50,-10}},
              lineColor={0,0,127},
              textString="signal-angular-velocity w"),Text(
              extent={{-70,70},{70,50}},
              lineColor={0,0,127},
              textString="parameter-angular-velocity w0"),Line(points={{-90,0},
            {-60,0}}, color={0,0,127}),Text(
              extent={{-20,40},{20,20}},
              lineColor={0,0,127},
              textString="or")}),
    ...
    annotation (
      ...
      Diagram(coordinateSystem(
          ...
          grid={2,2}), graphics={Text(
              extent={{-50,10},{50,-10}},
              lineColor={0,0,127},
              textString="signal-torque tau"),Text(
              extent={{-70,70},{70,50}},
              lineColor={0,0,127},
              textString="parameter-torque tau0"),Line(points={{-90,0},{-60,0}},
            color={0,0,127}),Text(
              extent={{-20,40},{20,20}},
              lineColor={0,0,127},
              textString="or")}),
...
"));
  ...
  model TabTimeTorque "Torque using table (time... torque)"
    ...
    parameter Integer direction(
      ...
      max=1) = 1 "forward or backward in time" annotation (choices(choice=1
          "t_first --> t_last", choice=-1 "t_first <-- t_last"));
  ...
  protected
    SI.Time t;
    final parameter SI.Time t_factor=if scale then T/abs(t_bd[2] - t_bd[1])
         else t_unit;
    final parameter SI.Torque tau_factor=if scale then drive_load*tau_scale*
        tau_unit else drive_load*tau_unit;
  ...
  equation
    ...
    annotation (defaultComponentName="tabTorq1", Documentation(info="<html>
...
<p>Note: start integration at time = 0</p>
  ...
  model TabPosSlopeTorque "Torque using table (position... slope)"
    ...
    parameter Real[2] s_bd(each unit="m") = {0,1}
      "{first, last} position in tab";
    parameter Integer dirTrack(
      min=-1,
      max=1) = +1 "forward or backward track" annotation (choices(choice=1
          "first-pos ---> last-pos", choice=-1 "first-pos <--- last-pos"));
    parameter Integer dirVehicle(
      min=-1,
      max=1) = +1 "vehicle forward or backward driving"
      annotation (choices(choice=1 "forward", choice=-1 "backward"));
  ...
  protected
    final parameter Real s_factor=if scale then D/abs(s_bd[2] - s_bd[1]) else
        s_unit;
  ...
  equation
    ...
    sin_gam = slope/sqrt(1 + slope*slope);
    // = sin(atan(slope))
    der(s) = sig*vVehicle;
    mass*der(vVehicle) = -(f + sig*mass*g_n*sin_gam + (cFrict[1]*cFrictUnit1 +
      cFrict[2]*cFrictUnit2*abs(vVehicle))*vVehicle);
...
"),
  ...
  equation
    phi = flange.phi;
    w = der(phi);
    flange.tau = (cFrict[1]*cFrictUnit1 + cFrict[2]*cFrictUnit2*noEvent(abs(w)))
      *w;
    annotation (
      defaultComponentName="frictTorq1",
      Documentation(info="<html>
...
<pre>
  ...
  w     angular velocity
      ...
      Icon(coordinateSystem(
          preserveAspectRatio=false,
          extent={{-100,-100},{100,100}},
          grid={2,2}), graphics={Polygon(points={{-38,20},{-38,20}}, lineColor=
            {255,250,110}),Ellipse(
              extent={{-60,60},{60,-60}},
              lineColor={135,135,135},
              fillColor={135,135,135},
              fillPattern=FillPattern.Solid),Line(points={{-80,0},{-60,0}},
  ...
  equation
    flange.phi = phi0;
    annotation (
      defaultComponentName="fixAng1",
      Icon(coordinateSystem(
          preserveAspectRatio=false,
          extent={{-100,-100},{100,100}},
          grid={2,2}), graphics={Line(points={{60,-40},{-80,-40},{20,60}},
            color={0,0,0}),Polygon(
...
"));
  ...
  model ThermalTurbineRotor "Thermal turbine rotor"
    ...
    annotation (
      defaultComponentName="turbRotor1",
      Documentation(info="<html>
...
"),
      Icon(coordinateSystem(
          ...
          grid={2,2}), graphics={Polygon(
              ...
              lineColor={176,0,0},
              fillColor={176,0,0},
              fillPattern=FillPattern.Solid),Line(points={{0,-70},{0,-50}},
...
"),
      Icon(coordinateSystem(
          ...
          grid={2,2}), graphics={Rectangle(
              ...
              fillColor={175,175,175},
              fillPattern=FillPattern.Solid),Polygon(
              points={{0,-120},{0,120},{8,80},{16,40},{16,20},{12,6},{0,0},{-12,
              -6},{-16,-20},{-16,-40},{-10,-80},{0,-120}},
              lineColor={0,0,0},
              fillColor={255,255,255},
              fillPattern=FillPattern.Solid),Rectangle(
              extent={{40,10},{100,-10}},
...
"),
  ...
  equation
    flange_a.phi + flange_b.phi = 2*phi;
    w = der(phi);
    a = der(w);
    J*a = flange_a.tau + flange_b.tau;
    d_tau = stiff*d_phi;
...
The parameter <tt>stiffness</tt> is a length-independent specification, in contrast to a spring-constant.</p>
  ...
  equation
    flange_a.phi = ratio_pn*flange_b.phi;
...
"),
      ...
      Diagram(coordinateSystem(
          ...
          grid={2,2}), graphics={Rectangle(
              ...
              fillColor={255,255,255},
              fillPattern=FillPattern.Solid,
              textString="ratio = {4, 2, 1}  or  {6000, 3000, 1500}"),Text(
              extent={{-70,-30},{-10,-50}},
              lineColor={0,0,0},
              textString="J[1] = 0"),Text(
              extent={{-30,100},{30,80}},
              lineColor={0,0,0},
              textString="J[2] = 0"),Text(
              extent={{10,-50},{70,-70}},
              lineColor={0,0,0},
              textString="J[3] = 0")}));
  end GearNoMass;
  ...
  protected
    final parameter Real ratio_pn=ratio[1]/ratio[end];
    final parameter Real[size(ratio, 1)] ratio2=diagonal(ratio)*ratio/(ratio[
        end]*ratio[end]);
...
"),
      ...
      Diagram(coordinateSystem(
          ...
          grid={2,2}), graphics={Text(
              ...
              fillColor={255,255,255},
              fillPattern=FillPattern.Solid,
              textString="ratio = {4, 2, 1}  or  {6000, 3000, 1500}")}));
  end Gear;
...
"),
      Icon(coordinateSystem(
          ...
          grid={2,2}), graphics={Rectangle(
              ...
              fillColor={255,255,255},
              fillPattern=FillPattern.Solid),Line(points={{-90,0},{90,0}},
            color={0,0,0})}));
  end NoGear;
  ...
  model AngleSensor "Angle and angular velocity sensor (mechanical)"
    ...
    Modelica.Blocks.Interfaces.RealOutput phi "angle" annotation (Placement(
          transformation(
          origin={-40,100},
          extent={{-10,-10},{10,10}},
          rotation=90)));
    Modelica.Blocks.Interfaces.RealOutput w "angular velocity" annotation (
...
"),
      Icon(coordinateSystem(
          ...
          grid={2,2}), graphics={Ellipse(
            ...
            points={{0,0},{9.02,28.6}}, color={95,95,95}),Polygon(
...
"),
      Icon(coordinateSystem(
          ...
          grid={2,2}), graphics={Ellipse(
              ...
              extent={{-100,-100},{100,-140}},
              lineColor={0,0,0},
              textString="%name"),Line(points={{-90,0},{-20,0}}, color={95,95,
            95}),Line(points={{0,0},{90,0}}, color={95,95,95}),Line(points={{30,
            20},{70,0},{30,-20}}, color={95,95,95}),Ellipse(extent={{-20,20},{
            20,-20}}, lineColor={135,135,135}),Line(points={{-66,24},{-48,18}},
            color={95,95,95}),Line(points={{-40,57},{-30,43}}, color={95,95,95}),
            Line(points={{0,70},{0,52}}, color={95,95,95}),Line(points={{40,57},
            {30,42}}, color={95,95,95}),Line(points={{66,24},{48,18}}, color={
            95,95,95})}));
  end PowerSensor;
...
"),
        Icon(coordinateSystem(
            ...
            grid={2,2}), graphics={Rectangle(extent={{-40,-90},{40,-70}},
              lineColor={128,128,128}),Line(points={{-40,-80},{40,-80}}, color=
              {128,128,128}),Line(points={{-20,-70},{-20,-90}}, color={128,128,
              128}),Ellipse(
                  extent={{-60,60},{60,-60}},
                  lineColor={0,0,0},
                  fillColor={175,175,175},
    ...
    partial model RigidRotorBase "Rigid rotor base"
      ...
      parameter SI.Inertia J=1 "inertia";
      parameter SI.AngularVelocity w_start=0 "start value of angular velocity"
    ...
    partial model RigidRotorCase "Rigid rotor with case"
      ...
      Interfaces.Rotation_p rotor
        "connector to turbine (mech) or airgap (el) torque" annotation (
          Placement(transformation(
            origin={0,60},
            extent={{10,-10},{-10,10}},
            rotation=270)));
      Interfaces.Rotation_p stator "access for stator reaction moment"
        annotation (Placement(transformation(
            ...
            extent={{-10,10},{10,-10}},
            rotation=180)));
      Interfaces.Rotation_n friction "access for friction model" annotation (
          Placement(transformation(
            origin={0,-80},
            extent={{-10,-10},{10,10}},
            rotation=90)));
...
"), Diagram(coordinateSystem(
  ...
  package Ports "One- and two-flange base for rotating mechanical components."
    ...
    equation
      flange_b.phi - flange_a.phi = d_phi;
      flange_b.tau - flange_a.tau = 2*d_tau;
      annotation (Documentation(info="<html>
</html>"));
    end Compliant;
