...
package Transforms "Transform functions"
  ...
  constant Real[3, 3] Park0=[[2, -1, -1]/sqrt(6); [0, 1, -1]/sqrt(2); [1, 1, 1]
      /sqrt(3)] "Orthogonal transform = Park(theta=0)";
...
<p><a href=\"modelica://PowerSystems.UsersGuide.Introduction.Transforms\">up users guide</a></p>
  ...
  algorithm
    y := cat(
        1,
        {-u[2, :],u[1, :]},
        zeros(size(u, 1) - 2, size(u, 2)));
    // Dymola implementation now ok?
...
It acts on the first index in the same way as j_dq0 for all values of the second index.
  ...
  algorithm
    ...
    annotation (
      Inline=true,
      derivative=PowerSystems.Utilities.Transforms.der_park,
      Documentation(info="<html>
...
<p><a href=\"modelica://PowerSystems.UsersGuide.Introduction.Transforms\">up users guide</a></p>
  ...
  algorithm
    ...
    annotation (
      Inline=true,
      derivative(order=2) = PowerSystems.Utilities.Transforms.der2_park,
      Documentation(info="<html>
<p>First derivative of function park(theta) with respect to time.</p>
  ...
  algorithm
    ...
    c := cos({theta,theta - dph_b,theta - dph_c});
    s := sin({theta,theta - dph_b,theta - dph_c});
    der2_P := transpose([-s23omega_dot*s - s23omega2*c, -s23omega_dot*c +
      s23omega2*s, {0,0,0}]);
...
<p>Second derivative of function park(theta) with respect to time.</p>
  ...
  algorithm
    ...
    annotation (
      Inline=true,
      derivative=PowerSystems.Utilities.Transforms.der_rotation_dq,
      Documentation(info="<html>
...
<p>Second derivative of function rotation_dq(theta) with respect to time.</p>
  ...
  algorithm
    ...
    annotation (
      Inline=true,
      derivative=PowerSystems.Utilities.Transforms.der_rotation_abc,
      Documentation(info="<html>
...
<p>First derivative of function rotation_abc(theta) with respect to time.</p>
  ...
  algorithm
    ...
    d2g := omega*omega*{ac - c,ac - bs,ac + bs} + omega_dot*{as - s,as + bc,as
       - bc};
...
<p><a href=\"modelica://PowerSystems.UsersGuide.Introduction.Transforms\">up users guide</a></p>
  ...
  algorithm
    ...
    annotation (
      Inline=true,
      derivative(order2) = PowerSystems.Utilities.Transforms.der2_permutation,
      Documentation(info="<html>
