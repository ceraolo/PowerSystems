...
package Math "Mathematical functions"
  ...
  function atanVarCut "arc-tangens with variable cut"
    ...
    output SI.Angle phi "arc(x) with range (alpha-pi) < phi <= (alpha+pi)";
  protected
    Real c;
    Real s;
...
"));
  ...
  algorithm
    ...
    for k in n:-1:1 loop
      c[n:-1:k, :] := c[n:-1:k, :] - cat(
          2,
          r[k, 1]*c[n + 1:-1:k + 1, 1:1] - r[k, 2]*c[n + 1:-1:k + 1, 2:2],
          r[k, 1]*c[n + 1:-1:k + 1, 2:2] + r[k, 2]*c[n + 1:-1:k + 1, 1:1]);
    end for;
    annotation (Documentation(info="<html>
...
"));
  ...
  algorithm
    ...
    fv := zeros(n + 1);
    v[:, 1] := x0;
    //Place input guess in the simplex
    fv[1] := fcn(x0, x_opt);
    funcount := 1;
    itercount := 0;
    ...
    for j in 1:n loop
      x1 := x0;
      x1[j] := if abs(x1[j]) > Modelica.Constants.small then (1 + delta)*x1[j]
         else delta0;
      v[:, j + 1] := x1;
      fv[j + 1] := fcn(x1, x_opt);
    end for;
    ...
    // Main algorithm
    while (funcount < max_fun and itercount < max_iter) and (max(abs(fv[ones(n)]
         - fv[2:n + 1])) > tol_f or max(abs(v[:, ones(n)] - v[:, 2:n + 1])) >
        tol_x) loop

      // Compute the reflection point
      xbar := sum(v[:, k] for k in 1:n)/n;
      //average of the n (NOT n+1) best points
...
Should be modified (domains with boundaries).</p>
  ...
  algorithm
    y[2] := if t < 0 then 1 else if t < t_char then (exp(-0.5*dt^beta) - gamma)
      /(1 - gamma) else 1;
    y[1] := 1 - y[2];
    annotation (smoothOrder=0, Documentation(info="<html>
