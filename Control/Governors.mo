...
package Governors "Turbine Governors "
  ...
  block GovernorSimple "Simple governor for constant turbine power"
    ...
    parameter Boolean par=true "p parameter or initialised?" annotation (
        Evaluate=true, choices(choice=true "parameter", choice=false
          "initialised"));
    parameter SIpu.Power p(
      unit="1",
      fixed=par) = 1 "turbine power" annotation (Dialog(enable=par));
    Modelica.Blocks.Interfaces.RealInput speed(final unit="1")
      "speed of turbine pu" annotation (Placement(transformation(
          origin={-60,-100},
          extent={{-10,-10},{10,10}},
          rotation=90)));
    Modelica.Blocks.Interfaces.RealOutput power(final unit="1")
      "power of turbine pu" annotation (Placement(transformation(
          origin={60,-100},
          extent={{-10,-10},{10,10}},
          rotation=270)));
...
"),   Diagram(coordinateSystem(
          ...
          grid={2,2}), graphics={Rectangle(extent={{0,20},{20,0}}, lineColor={
            95,95,95}),Line(points={{20,10},{60,10},{60,10},{60,-100}}, color={
            95,95,95}),Text(
              extent={{0,20},{20,0}},
              lineColor={95,95,95},
              fillColor={127,0,255},
  ...
  equation
    connect(setptPower, limiter.u) annotation (Line(points={{-100,-40},{-26,-40},
            {-26,0},{48,0}}, color={0,0,127}));
    annotation (
      defaultComponentName="governor",
      Icon(coordinateSystem(
          preserveAspectRatio=false,
...
"),   Icon(coordinateSystem(
  ...
  package Partials "Partial models"
    ...
    partial block GovernorBase "Governor base"
      ...
      Modelica.Blocks.Interfaces.RealInput setptPower(final unit="1")
        "setpoint power pu"
        annotation (Placement(transformation(extent={{-110,-50},{-90,-30}})));
      Modelica.Blocks.Interfaces.RealInput setptSpeed(final unit="1")
        "setpoint speed pu"
      ...
      Modelica.Blocks.Interfaces.RealOutput power(final unit="1")
        "turbine power pu" annotation (Placement(transformation(
            ...
            extent={{-10,-10},{10,10}},
            rotation=270)));
      Modelica.Blocks.Nonlinear.Limiter limiter(uMin=p_minmax[1], uMax=p_minmax[
            2])
    ...
    equation
      ...
      annotation (defaultComponentName="governor", Documentation(info="<html>
</html>"));
    end GovernorBase;
