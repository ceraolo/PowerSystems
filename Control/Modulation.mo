  ...
  gates[5:6]     phase-module c
      ...
      Icon(coordinateSystem(
          ...
          grid={2,2}), graphics={Line(points={{0,-60},{0,-80}}, color={255,0,
            255}),Line(points={{-20,-60},{-20,-80}}, color={255,0,255}),Line(
            points={{20,-60},{20,-80}}, color={255,0,255})}));
  end PWMasyn;
    ...
    for k in 1:3 loop
      ...
      when phi[k] > phi0[pre(n[k])] + pre(sigdel_phi[k])*v_abc[k] then
        if noEvent(phi[k] < phi0[pre(n[k])] + delp_phi) then
          ...
          gates[{pgt[k],ngt[k]}] = pre(gates[{pgt[k],ngt[k]}]);
          sigdel_phi[k] = pre(sigdel_phi[k]);
          n[k] = if pre(n[k]) < m2 - 1 then pre(n[k]) + 2 else pre(n[k]) + 2 -
            m2;
        end if;
      end when;
    end for;
...
"),
      Icon(coordinateSystem(
          ...
          grid={2,2}), graphics={Line(points={{-20,-60},{-20,-80}}, color={255,
            0,255}),Line(points={{0,-60},{0,-80}}, color={255,0,255}),Line(
            points={{20,-60},{20,-80}}, color={255,0,255})}));
  end PWMsyn;

  block PWMtab "PWM tabulated, synchronous mode, 3phase"
    extends Partials.PWMsynBase(final m=3, final mult=integer((size(table.phiIgn,
          2) + 1)/4));

    replaceable PowerSystems.Control.IgnitionTables.Table_2 table
      "table(multiplicity)" annotation (choices(
...
"),
  ...
  equation
    u = min(vPhasor[1], 0.85);
    // preliminary (no overmodulation)
...
"),
      Icon(coordinateSystem(
          ...
          grid={2,2}), graphics={Line(points={{-20,-60},{-20,-80}}, color={255,
            0,255}),Line(points={{0,-60},{0,-80}}, color={255,0,255}),Line(
            points={{20,-60},{20,-80}}, color={255,0,255}),Line(points={{-80,60},
            {80,-60}}, color={255,0,0})}));
  end SVPWMasyn;
...
"),
  ...
  block SVPWM "Space vector PWM"
    ...
    Modelica.Blocks.Math.Add phaseCorr annotation (Placement(transformation(
          origin={-60,30},
          extent={{10,-10},{-10,10}},
          rotation=90)));
  equation
    phaseCorr.u2 = der(theta)/(2*f_carr);
    //d_phi = pi*f/f_carr.
    ...
    connect(phToAlphaBeta.y, logic.u_alpha_beta)
      annotation (Line(points={{-50,0},{-41.3333,0}}, color={0,0,127}));
    connect(logic.trigger, pwm.trigger)
      annotation (Line(points={{1.33333,8},{18,8}}, color={255,0,255}));
    connect(pwm.switch, gates) annotation (Line(points={{62,0},{80,0},{80,-40},
            {-60,-40},{-60,-100}}, color={255,0,255}));
    connect(logic.dutyRatio, pwm.dutyRatio)
      annotation (Line(points={{1.33333,-8},{18,-8}}, color={0,0,127}));
    connect(theta, phaseCorr.u1) annotation (Line(points={{-60,100},{-60,60},{-66,
            60},{-66,42}}, color={0,0,127}));
    connect(phaseCorr.y, phToAlphaBeta.theta)
      annotation (Line(points={{-60,19},{-60,10}}, color={0,0,127}));
    annotation (
      defaultComponentName="sv_pwm",
      Documentation(info="<html>
...
"),
      ...
      Diagram(coordinateSystem(
          preserveAspectRatio=false,
          extent={{-100,-100},{100,100}},
          grid={2,2}), graphics={Rectangle(extent={{-40,65},{0,55}}, lineColor=
            {128,0,255}),Text(
              extent={{-40,65},{0,55}},
              lineColor={128,0,255},
              textString="d_phi"),Line(points={{-40,60},{-54,60},{-54,50}},
            color={128,0,255})}));
  end SVPWM;
...
*/
    ...
    when phi > phi0[pre(n)] + pre(sigdel_phi)*u then
      if noEvent(phi < phi0[pre(n)] + delp_phi) then
        ...
        gates = pre(gates);
        sigdel_phi = pre(sigdel_phi);
        n = if pre(n) < m2 - 1 then pre(n) + 2 else pre(n) + 2 - m2;
      end if;
    end when;
    annotation (
      defaultComponentName="pwm",
...
"),
  ...
  block PWMtab1ph "PWM tabulated, synchronous mode, 1-phase"
    extends Partials.PWMsynBase(final m=2, final mult=integer((size(table.phiIgn,
          2) + 1)/4));
...
"),
      Icon(coordinateSystem(
          ...
          grid={2,2}), graphics={Line(points={{-20,56},{-20,36}}, color={128,
            128,128}),Line(points={{-40,46},{40,46}}, color={128,128,128}),
            Rectangle(extent={{-40,36},{40,56}}, lineColor={128,128,128}),Line(
            points={{0,-60},{0,-80}}, color={255,0,255})}));
  end PWMtab1ph;
...
"),
  ...
  equation
    ...
    annotation (
      defaultComponentName="chopperMod",
      Documentation(info="<html>
...
"),
  ...
  package Partials "Partial models"
    ...
    partial block ModulatorBase "Modulator base"
      ...
      parameter Integer m(
        min=2,
        max=3) = 3 "3 for 3-phase, 2 for 1-phase";
      Modelica.Blocks.Interfaces.RealInput theta "reference angle" annotation (
          Placement(transformation(
            origin={-60,100},
            extent={{-10,-10},{10,10}},
            rotation=270)));
      Modelica.Blocks.Interfaces.RealInput[2] vPhasor
        "voltage demand pu {norm(v), phase(v)} (see info)" annotation (
          Placement(transformation(
            origin={60,100},
            extent={{-10,-10},{10,10}},
            rotation=270)));
      Modelica.Blocks.Interfaces.BooleanOutput[2*m] gates(each start=false)
        "gates" annotation (Placement(transformation(
            origin={-60,-100},
            extent={{-10,-10},{10,10}},
            rotation=270)));
    protected
    ...
    partial block SVPWMbase "Space vector PWM base"
      ...
      parameter Boolean sym=true "'symmetric' or 'antisymmetric'" annotation (
          Evaluate=true, choices(choice=true " true: symmetric", choice=false
            "false: antisymmetric"));
    protected
      Boolean[8, 3] state=[true, false, false; true, true, false; false, true,
          false; false, true, true; false, false, true; true, false, true;
          false, false, false; true, true, true];
      annotation (Icon(coordinateSystem(
            preserveAspectRatio=false,
            extent={{-100,-100},{100,100}},
            grid={2,2}), graphics={Polygon(points={{-30,-52},{30,-52},{60,0},{
              30,52},{-30,52},{-60,0},{-30,-52}}, lineColor={255,255,0}),Line(
              points={{0,0},{60,0}}, color={255,255,0}),Line(points={{0,0},{30,
              52}}, color={255,255,0}),Line(points={{0,0},{-30,52}}, color={255,
              255,0}),Line(points={{0,0},{-60,0}}, color={255,255,0}),Line(
              points={{0,0},{-30,-52}}, color={255,255,0}),Line(points={{0,0},{
              30,-52}}, color={255,255,0}),Line(
                  points={{0,0},{32,26}},
                  color={0,127,127},
...
*/
    ...
    protected
      ...
      SI.Angle[m2] Phi;
      SI.Angle[m2] Dphi;
      Integer[6, :] K=if sym then [1, 7, 1, 2, 8, 2; 2, 8, 2, 3, 7, 3; 3, 7, 3,
          4, 8, 4; 4, 8, 4, 5, 7, 5; 5, 7, 5, 6, 8, 6; 6, 8, 6, 1, 7, 1] else [
          1, 7, 8, 2, 0, 0; 2, 8, 7, 3, 0, 0; 3, 7, 8, 4, 0, 0; 4, 8, 7, 5, 0,
          0; 5, 7, 8, 6, 0, 0; 6, 8, 7, 1, 0, 0];
      Integer imax=if sym then 6 else 4;
      // size(K,2);
    equation
      if sym then
        Phi[1] = 0;
        ...
        for m in 1:m1 - 1 loop
          ...
          Dphi[3*m - 1:3*m + 1] = {-dphi[m] - dphi[m1 - m],(-1)^m*(dphi[m] -
            dphi[m1 - m]),dphi[m] + dphi[m1 - m]};
        end for;
        Phi[m2] = pi/3;
        Dphi[m2] = -dphi[m1];
      else
        ...
        for m in 1:m1 - 1 loop
          ...
          Dphi[4*m - 1:4*m + 1] = {-dphi[m] - dphi[m1 - m],(dphi[m] - dphi[m1
             - m]),dphi[m] + dphi[m1 - m]};
    ...
    partial block PWMasynBase "Sine PWM asynchronous base"
      ...
      parameter SI.Frequency f_carr=1e3 "carrier frequency"
        annotation (Evaluate=true);
      parameter SI.Time dt_min(
        min=Modelica.Constants.eps,
        max=0.1*del_t) = 1e-6 "minimal time between on/off"
        annotation (Evaluate=true);
    protected
      final parameter SI.Time del_t=1/(4*f_carr) annotation (Evaluate=true);
      final parameter SI.Time delp_t=del_t - dt_min/2 annotation (Evaluate=true);
      annotation (Icon(coordinateSystem(
            preserveAspectRatio=false,
            extent={{-100,-100},{100,100}},
            grid={2,2}), graphics={Line(points={{-60,-30},{-40,30},{-20,-30},{0,
              30},{20,-30},{40,30},{60,-30}}, color={255,255,0}),Line(points={{
              -60,-8},{-50,0},{-40,6},{-30,10},{-20,12},{-6,14},{6,14},{20,12},
              {30,10},{40,6},{50,0},{60,-8}}, color={0,127,127})}),
          Documentation(info="<html>
</html>"));
    ...
    partial block PWMsynBase "Sine PWM synchronous base"
      ...
      parameter Integer mult(min=1) = 3 "multiplicity (2*mult-1 pulses/period)"
        annotation (Evaluate=true);
      parameter SI.Angle dphi_min(
        min=Modelica.Constants.eps,
        max=0.1*del_phi) = 1e-6 "minimal time between on/off"
        annotation (Evaluate=true);
    protected
      final parameter Integer m2=2*(2*mult - 1) annotation (Evaluate=true);
    ...
    partial block BlockBase "Block modulator base"
      ...
      parameter SI.Angle dphimin(
        min=Modelica.Constants.eps,
        max=pi) = 1e-3 "minimal phase angle on" annotation (Evaluate=true);
      Modelica.Blocks.Interfaces.RealInput theta "reference angle" annotation (
          Placement(transformation(
            origin={-60,100},
            extent={{-10,-10},{10,10}},
    ...
    protected
      ...
      annotation (Icon(coordinateSystem(
            preserveAspectRatio=false,
            extent={{-100,-100},{100,100}},
            grid={2,2}), graphics={Line(points={{-70,-30},{-60,-30},{-60,30},{-30,
              30},{-30,-30},{0,-30},{0,30},{30,30},{30,-30},{60,-30},{60,30},{
              70,30}}, color={0,120,120}),Line(points={{-80,0},{80,0}}, color={
              0,0,0}),Rectangle(
                  extent={{-80,120},{-40,80}},
                  lineColor={213,170,255},
                  fillColor={213,170,255},
...
<br>
    ...
    model SVMpwm "Block generating the pwm pulses for SVM"
      ...
      Modelica.Blocks.Math.Add stop_001
        annotation (Placement(transformation(extent={{20,-186},{40,-166}})));
      ...
      Modelica.Blocks.Discrete.TriggeredSampler load_data[3] annotation (
          Placement(transformation(
            origin={-190,-110},
            extent={{10,-10},{-10,10}},
            rotation=90)));
      ...
      Modelica.Blocks.Logical.Not Not annotation (Placement(transformation(
            ...
            extent={{-10,-10},{10,10}},
            rotation=270)));
      DutyRatioToCycle dutyRtoC annotation (Placement(transformation(extent={{-190,
                -160},{-170,-140}})));
    ...
    equation
      connect(one.y, t1.u1)
        annotation (Line(points={{-129,-90},{-108,-90}}, color={0,0,127}));
      ...
      connect(t2.y, start_010.u)
        annotation (Line(points={{-91,-130},{-82,-130}}, color={0,0,127}));
      connect(t3.y, start_001.u)
        annotation (Line(points={{-91,-170},{-82,-170}}, color={0,0,127}));
      connect(one.y, t2.u1) annotation (Line(points={{-129,-90},{-120,-90},{-120,
              -130},{-108,-130}}, color={0,0,127}));
      ...
      connect(start_100.y, stop_100.u1)
        annotation (Line(points={{-59,-90},{18,-90}}, color={0,0,127}));
      connect(start_010.y, stop_010.u1)
        annotation (Line(points={{-59,-130},{18,-130}}, color={0,0,127}));
      connect(stop_010.u2, DeMultiplex3_1.y2[1]) annotation (Line(points={{18,-142},
              {10,-142},{10,-150},{-139,-150}}, color={0,0,127}));
      connect(start_001.y, stop_001.u1)
        annotation (Line(points={{-59,-170},{18,-170}}, color={0,0,127}));
      connect(stop_001.u2, DeMultiplex3_1.y3[1]) annotation (Line(points={{18,-182},
              {10,-182},{10,-190},{-130,-190},{-130,-157},{-139,-157}}, color={
              0,0,127}));
      connect(stop_010.y, GreaterEqual5.u2) annotation (Line(points={{41,-136},
              {60,-136},{60,62},{78,62}}, color={0,0,127}));
      connect(dutyRatio, load_data.u) annotation (Line(points={{-220,-80},{-190,
              -80},{-190,-98}}, color={0,0,127}));
      ...
      connect(GreaterEqual2.u1, GreaterEqual3.u1) annotation (Line(points={{-2,
              90},{-60,90},{-60,10},{-2,10}}, color={0,0,127}));
      connect(GreaterEqual3.u1, GreaterEqual6.u1) annotation (Line(points={{-2,
              10},{-60,10},{-60,-40},{40,-40},{40,-10},{78,-10}}, color={0,0,
              127}));
      connect(GreaterEqual6.u1, GreaterEqual5.u1) annotation (Line(points={{78,
              -10},{40,-10},{40,70},{78,70}},color={0,0,127}));
      connect(GreaterEqual5.u1, GreaterEqual4.u1) annotation (Line(points={{78,
              70},{40,70},{40,150},{78,150}}, color={0,0,127}));
      ...
      connect(GreaterEqual4.y, Not1.u)
        annotation (Line(points={{101,150},{118,150}}, color={255,0,255}));
      connect(GreaterEqual1.y, And1.u1)
        annotation (Line(points={{21,170},{158,170}}, color={255,0,255}));
      connect(Not1.y, And1.u2) annotation (Line(points={{141,150},{150,150},{
              150,162},{158,162}}, color={255,0,255}));
      connect(Not2.y, And2.u2) annotation (Line(points={{141,70},{150,70},{150,
              82},{158,82}}, color={255,0,255}));
      connect(Not3.y, And3.u2) annotation (Line(points={{141,-10},{150,-10},{
              150,2},{158,2}}, color={255,0,255}));
      ...
      connect(GreaterEqual2.y, And2.u1)
        annotation (Line(points={{21,90},{158,90}}, color={255,0,255}));
      connect(GreaterEqual5.y, Not2.u)
        annotation (Line(points={{101,70},{118,70}}, color={255,0,255}));
      connect(And1.y, Not4.u) annotation (Line(points={{181,170},{184,170},{184,
              150},{158,150},{158,130}},color={255,0,255}));
      connect(And2.y, Not5.u) annotation (Line(points={{181,90},{184,90},{184,
              68},{158,68},{158,50}},color={255,0,255}));
      connect(And3.y, Not6.u) annotation (Line(points={{181,10},{184,10},{184,-10},
              {158,-10},{158,-30}}, color={255,0,255}));
      connect(And1.y, switch[1]) annotation (Line(points={{181,170},{200,170},{
              200,22},{220,22},{220,-16.6667}}, color={255,0,255}));
      connect(Not4.y, switch[2]) annotation (Line(points={{181,130},{200,130},{
              200,-10},{220,-10}}, color={255,0,255}));
      connect(And2.y, switch[3]) annotation (Line(points={{181,90},{200,90},{
              200,-3.33333},{220,-3.33333}}, color={255,0,255}));
      connect(Not5.y, switch[4]) annotation (Line(points={{181,50},{200,50},{
              200,3.33333},{220,3.33333}}, color={255,0,255}));
      connect(And3.y, switch[5]) annotation (Line(points={{181,10},{190.5,10},{
              190.5,10},{220,10}}, color={255,0,255}));
      connect(Not6.y, switch[6]) annotation (Line(points={{181,-30},{200,-30},{
              200,16.6667},{220,16.6667}}, color={255,0,255}));
      ...
      connect(dutyRtoC.dutyCycle, DeMultiplex3_1.u)
        annotation (Line(points={{-168,-150},{-162,-150}}, color={0,0,127}));
      annotation (
        Icon(coordinateSystem(preserveAspectRatio=false, extent={{-200,-200},{
                200,200}}), graphics={Rectangle(
                  ...
                  fillColor={255,255,255},
                  fillPattern=FillPattern.Solid,
                  textString="a0"),Line(points={{150,120},{150,-140}}, color={0,
              0,255}),Line(
                  points={{-30,140},{-30,-80}},
                  color={215,215,215},
                  thickness=0.5),Line(
                  ...
                  fillColor={255,255,255},
                  fillPattern=FillPattern.Solid,
                  textString="c0"),Line(points={{-190,80},{-130,80},{-130,120},
              {110,120},{110,80},{170,80}}, color={0,0,255}),Line(points={{-190,
              0},{-90,0},{-90,40},{50,40},{50,0},{170,0}}, color={0,0,255}),
              Line(points={{-190,-80},{-30,-80},{-30,-40},{10,-40},{10,-80},{
              168,-80}}, color={0,0,255}),Line(points={{-170,140},{-170,-140}},
              color={0,0,255}),Text(extent={{-202,360},{200,200}}, textString=
              "%name")}),
...
A duty ratio a=1 indicates a continuously closed upper switch of the first inverter leg. At a duty ratio a=0, the turn-on time during each PWM period is equally distributed to the lower and upper switch and the resulting mean value of the phase voltage u_a0 is zero. At a duty ratio a=-1, the lower switch is continuously closed, etc.</p>
    ...
    equation
      ...
      connect(u_beta, Gain1.u) annotation (Line(points={{-110,-30},{-70,-30},{-70,
              50},{-62,50}}, color={0,0,127}));
      connect(Gain1.y, Add1.u2) annotation (Line(points={{-39,50},{-20,50},{-20,
              54},{-2,54}},color={0,0,127}));
      connect(Add1.y, Gain2.u) annotation (Line(points={{21,60},{40,60},{40,-60},
              {58,-60}}, color={0,0,127}));
      connect(u_alpha, Gain3.u) annotation (Line(points={{-110,30},{-80,30},{-80,
              10},{-42,10}}, color={0,0,127}));
      connect(Gain3.y, Add2.u1) annotation (Line(points={{-19,10},{-10,10},{-10,
              6},{-2,6}}, color={0,0,127}));
      connect(u_beta, Gain4.u)
        annotation (Line(points={{-110,-30},{-42,-30}}, color={0,0,127}));
      connect(Gain4.y, Add2.u2) annotation (Line(points={{-19,-30},{-10,-30},{-10,
              -6},{-2,-6}}, color={0,0,127}));
      connect(Add1.y, abc[1]) annotation (Line(points={{21,60},{40,60},{40,0},{
      ...
      annotation (Icon(graphics={Rectangle(
                  ...
                  fillColor={255,255,0},
                  fillPattern=FillPattern.Solid,
                  textString="b = -ua + 3/sqrt(3) ub"),Text(
                  extent={{-72,40},{-16,0}},
                  lineColor={215,215,215},
                  fillColor={255,255,0},
                  fillPattern=FillPattern.Solid,
                  ...
                  fillColor={255,255,0},
                  fillPattern=FillPattern.Solid,
                  textString="a =  ua + 1/sqrt(3) ub"),Text(
                  extent={{-100,-10},{100,-90}},
                  lineColor={0,0,0},
                  textString="S1&4"),Text(extent={{-150,150},{150,110}},
              textString="%name")}));
    end SVMsector1p4;
    ...
    equation
      connect(u_beta, Gain4.u)
        annotation (Line(points={{-110,-30},{-42,-30}}, color={0,0,127}));
      connect(u_alpha, Gain1.u) annotation (Line(points={{-110,30},{-76,30},{-76,
              60},{-42,60}}, color={0,0,127}));
      connect(Gain4.y, Gain2.u) annotation (Line(points={{-19,-30},{40,-30},{40,
      ...
      annotation (Icon(graphics={Rectangle(
                  ...
                  fillColor={255,255,0},
                  fillPattern=FillPattern.Solid,
                  textString="b = 2 sqrt(3) ub"),Text(
                  extent={{-74,100},{6,60}},
                  lineColor={215,215,215},
                  fillColor={255,255,0},
                  fillPattern=FillPattern.Solid,
                  textString="a = 2 ua"),Text(extent={{-150,150},{150,110}},
              textString="%name")}));
    end SVMsector2p5;
    ...
    equation
      connect(u, Edge1.u)
        annotation (Line(points={{-110,0},{-82,0}}, color={255,0,255}));
      ...
      connect(Or1.y, y) annotation (Line(points={{-19,0},{25.5,0},{25.5,0},{110,
              0}}, color={255,0,255}));
      connect(Or1.u2, Pre1.y) annotation (Line(points={{-42,-8},{-50,-8},{-50,-39}},
            color={255,0,255}));
      connect(Pre1.u, And1.y) annotation (Line(points={{-50,-62},{-50,-80},{92,
              -80},{92,-30},{81,-30}},color={255,0,255}));
      ...
      annotation (Icon(graphics={Rectangle(
                  ...
                  lineColor={0,0,0},
                  fillColor={215,215,215},
                  fillPattern=FillPattern.Solid),Line(points={{-60,-90},{-46,-90}},
              color={135,135,135}),Line(points={{6,-90},{20,-90}}, color={135,
              135,135}),Polygon(points={{-46,-88},{-46,-92},{-40,-90},{-46,-88}},
              lineColor={135,135,135}),Polygon(points={{0,-90},{6,-88},{6,-92},
              {0,-90}}, lineColor={135,135,135}),Text(
                  extent={{-44,-80},{0,-102}},
                  ...
                  extent={{48,-90},{84,-98}},
                  lineColor={0,0,0},
                  textString="time"),Line(points={{-80,-64},{-40,-64},{-40,-4},
              {0,-4},{0,-64},{60,-64}}, color={255,0,255}),Line(points={{-80,20},
              {-40,20},{-40,80},{-40,80},{60,80},{60,80}}, color={255,0,255}),
              Polygon(
                  points={{-80,80},{-88,58},{-72,58},{-80,80}},
                  lineColor={0,0,0},
                  fillColor={255,255,255},
                  fillPattern=FillPattern.Solid),Line(points={{-80,58},{-80,-90}},
              color={0,0,0}),Line(points={{-90,-80},{82,-80}}, color={0,0,0}),
              Polygon(
