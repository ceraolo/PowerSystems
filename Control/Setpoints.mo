...
package Setpoints "Setpoints of generators"
  ...
  protected
    parameter SIpu.AngularVelocity w_set(
      unit="1",
      fixed=false,
      start=1) "setpt turbine speed pu";
    parameter SIpu.Power p_set(
      unit="1",
      fixed=false,
      start=1) "setpt turbine power pu";
    parameter SIpu.Voltage v_set(
      unit="1",
      fixed=false,
      start=1) "setpt exciter voltage pu";
...
"),   Icon(coordinateSystem(
  ...
  block Set_w_p "Set-points for generation"
    ...
    parameter SIpu.AngularVelocity w_set(
      unit="1",
      fixed=true) = 1 "setpoint turbine speed pu";
    Modelica.Blocks.Interfaces.RealInput setpt_p "setpoint turbine power pu"
      annotation (Placement(transformation(extent={{-30,-10},{-10,10}})));
    Modelica.Blocks.Interfaces.RealOutput[2] setpts
