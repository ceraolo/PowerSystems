...
package Exciters "Generator Exciters "
  ...
  block ExciterSimple "Simple exciter for constant field voltage"
    ...
    parameter Boolean par=true "v_f parameter or initialised?" annotation (
        Evaluate=true, choices(choice=true "parameter", choice=false
          "initialised"));
    parameter SIpu.Voltage v_f(
      unit="1",
      fixed=par) = 1 "exciter voltage" annotation (Dialog(enable=par));
...
"),   Icon(coordinateSystem(
  ...
  protected
    ...
    Blocks.Math.Norm norm(final n=3, n_eval=3) annotation (Placement(
          transformation(
          origin={-60,-70},
          extent={{-10,-10},{10,10}},
          rotation=90)));
  ...
  equation
    ...
    connect(termVoltage, norm.u)
      annotation (Line(points={{-60,-100},{-60,-80}}, color={0,0,127}));
    connect(norm.y, delta_voltage.u2) annotation (Line(points={{-60,-60},{-60,-40},
