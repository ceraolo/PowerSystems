...
"));
  ...
  partial model NominalAC "Units and nominal values AC"
    ...
    parameter SI.Frequency f_nom=system.f_nom "nominal frequency" annotation (
      Evaluate=true,
...
"));
  ...
  partial model NominalDC "Units and nominal values DC"
    ...
    parameter SI.AngularVelocity w_nom=system.w_nom "nominal r.p.m."
      annotation (
      Evaluate=true,
      Dialog(group="Nominal"),
      choices(
...
"));
  ...
  record NominalData "Units and nominal data"
    ...
    SI.Voltage V_nom(final min=0) = 1 "nominal Voltage (= base for pu)"
      annotation (Evaluate=true,Dialog(group="Nominal"));
    SI.ApparentPower S_nom(final min=0) = 1 "nominal Power (= base for pu)"
      annotation (Evaluate=true,Dialog(group="Nominal"));
  ...
  record NominalDataAC "Units and nominal data AC"
    ...
    SI.Frequency f_nom=50 "nominal frequency" annotation (
      Evaluate=true,
      Dialog(group="Nominal"),
      choices(
        choice=50/3 "50/3 Hz",
  ...
  record NominalDataDC "Units and nominal data DC"
    ...
    SI.AngularVelocity w_nom=314.159265358979323846 "nominal r.p.m."
      annotation (
      Evaluate=true,
      Dialog(group="Nominal"),
      choices(
  ...
  record NominalDataTrafo "Units and nominal data transformer"
    ...
    SI.Voltage[:] V_nom(each final min=0) = {1,1}
      ...
      annotation (Evaluate=true,Dialog(group="Nominal"));
    SI.ApparentPower S_nom(final min=0) = 1 "nominal Power (= base of pu)"
      annotation (Evaluate=true,Dialog(group="Nominal"));
    SI.Frequency f_nom=50 "nominal frequency" annotation (
      Evaluate=true,
      Dialog(group="Nominal"),
