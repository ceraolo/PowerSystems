...
*/
  ...
  equation
    ...
    r = relaxation(
        ...
        t_relax,
        p_relax);
    {v,i} = if closed then {(r[1] + r[2]*epsR)*s,(r[1]*epsG + r[2])*s} else {(r[
      1]*epsR + r[2])*s,(r[1] + r[2]*epsG)*s};
    annotation (
...
<pre>
  ...
  p           power of exponent
      ...
      Diagram(coordinateSystem(
          ...
          grid={2,2}), graphics={Line(
              ...
              textString=" forced switch with exponential relaxation")}));
  end ForcedSwitch;
  ...
  model ForcedCommSwitch "Forced commuting switch, 1-phase"
    ...
    parameter Real[2] eps(
      final min={0,0},
      each unit="1") = {1e-4,1e-4} "{resistance 'closed', conductance 'open'}";
    parameter SI.Time t_relax=10e-3 "switch relaxation time";
    parameter Integer p_relax(min=2) = 4 "power of relaxation exponent";
    PS.Voltage[2] v_t;
    PS.Voltage[2] v_f;
    ...
    Modelica.Blocks.Interfaces.BooleanInput control
      "true: p - nt closed, false: p - nf closed" annotation (Placement(
          transformation(
          origin={0,100},
          extent={{-10,-10},{10,10}},
          rotation=270)));
...
*/
  ...
  equation
    ...
    r = relaxation(
        ...
        t_relax,
        p_relax);
    {v_t,i_t} = if closed_t then {(r[1] + r[2]*epsR)*s_t,(r[1]*epsG + r[2])*s_t}
       else {(r[1]*epsR + r[2])*s_t,(r[1] + r[2]*epsG)*s_t};
    {v_f,i_f} = if open_t then {(r[1] + r[2]*epsR)*s_f,(r[1]*epsG + r[2])*s_f}
       else {(r[1]*epsR + r[2])*s_f,(r[1] + r[2]*epsG)*s_f};
    annotation (
      defaultComponentName="switch1",
      Documentation(info="<html>
<p>Allows switching of single conductor (of totally one or two).<br>
...
<pre>
  ...
  p           power of exponent
      ...
      Icon(coordinateSystem(
          ...
          grid={2,2}), graphics={Rectangle(
              ...
              fillPattern=FillPattern.Solid),Line(points={{-80,0},{-40,0},{50,
            40}}, color={0,0,255}),Line(
              points={{-40,0},{50,-40}},
              color={0,0,255},
              pattern=LinePattern.Dot),Line(points={{40,40},{80,40}}, color={0,
            0,255}),Text(
              extent={{-80,-40},{80,-80}},
              lineColor={0,0,255},
              textString="exp"),Line(points={{40,-40},{80,-40}}, color={0,0,255}),
            Line(
              points={{0,90},{0,20}},
              color={255,0,255},
              pattern=LinePattern.Dot),Text(
      ...
      Diagram(coordinateSystem(
          ...
          grid={2,2}), graphics={Line(
              ...
              fillColor={0,0,255},
              fillPattern=FillPattern.Solid,
              textString=" forced commuting switch with exponential relaxation"),
            Line(points={{-80,30},{-30,30}}, color={0,0,255}),Line(
              points={{12,20},{30,20}},
              color={0,0,255},
              thickness=0.5),Line(points={{30,40},{40,40},{40,46},{80,46}},
            color={0,0,255}),Line(points={{54,20},{60,20},{60,-34},{80,-34}},
            color={0,0,255}),Line(points={{30,-40},{50,-40},{50,-46},{80,-46}},
            color={0,0,255}),Line(
  ...
  protected
    Common.Switching.Switch switch_1(epsR=epsR, epsG=epsG)
      annotation (Placement(transformation(extent={{-40,0},{40,60}})));
...
*/
  ...
  equation
    ...
    r_2 = relaxation(
        ...
        t_relax,
        p_relax);
    {v[1],i[1]} = if closed[1] then {(r_1[1] + r_1[2]*epsR)*s[1],(r_1[1]*epsG
       + r_1[2])*s[1]} else {(r_1[1]*epsR + r_1[2])*s[1],(r_1[1] + r_1[2]*epsG)
      *s[1]};
    {v[2],i[2]} = if closed[2] then {(r_2[1] + r_2[2]*epsR)*s[2],(r_2[1]*epsG
       + r_2[2])*s[2]} else {(r_2[1]*epsR + r_2[2])*s[2],(r_2[1] + r_2[2]*epsG)
      *s[2]};
    annotation (
      defaultComponentName="switch1",
      Documentation(info="<html>
...
<pre>
  ...
  p           power of exponent
      ...
      Diagram(coordinateSystem(
          ...
          grid={2,2}), graphics={Line(
              ...
              fillColor={0,0,255},
              fillPattern=FillPattern.Solid,
              textString=" forced switching with exponential relaxation"),Line(
              points={{10,-30},{32,-30}},
              color={0,0,255},
              pattern=LinePattern.Dot),Line(
  ...
  protected
    Common.Switching.Switch switch_1(epsR=epsR, epsG=epsG)
      annotation (Placement(transformation(extent={{-60,0},{20,60}})));
    Common.Switching.Switch switch_2(epsR=epsR, epsG=epsG)
      annotation (Placement(transformation(extent={{-20,-60},{60,0}})));
  ...
  equation
    ...
    connect(control[2], switch_2.closed) annotation (Line(points={{0,105},{0,80},
            {20,80},{20,0}}, color={255,0,255}));
    annotation (
      defaultComponentName="switch1",
      Documentation(info="<html>
...
"),
  ...
  equation
    ...
    breaker_2.v = v[2];
    breaker_2.i = i[2];
    connect(control[1], breaker_1.closed) annotation (Line(points={{0,95},{0,80},
            {-20,80},{-20,60}}, color={255,0,255}));
...
"), Icon(coordinateSystem(
            ...
            grid={2,2}),graphics={Rectangle(
                  ...
                  lineColor={255,255,255},
                  fillColor={255,255,255},
                  fillPattern=FillPattern.Solid),Line(points={{40,10},{80,10}},
              color={0,0,255}),Line(points={{-80,10},{-40,10},{30,50}}, color={
              0,0,255}),Line(
                  points={{0,90},{0,34}},
                  color={255,0,255},
                  pattern=LinePattern.Dot)}));
...
"), Diagram(coordinateSystem(
            preserveAspectRatio=false,
            extent={{-100,-100},{100,100}},
            grid={2,2}), graphics={Line(points={{-80,-30},{80,-30}}, color={0,0,
              255}),Line(points={{-80,30},{-30,30}}, color={0,0,255}),Line(
              points={{30,30},{80,30}}, color={0,0,255})}));
    end SwitchBase;
    ...
    partial model DoubleSwitchBase "Double switch base, 1-phase"
      ...
      Modelica.Blocks.Interfaces.BooleanInput[2] control
        "true:closed, false:open" annotation (Placement(transformation(
            origin={0,100},
            extent={{-10,-10},{10,10}},
            rotation=270)));
...
"),
        ...
        Diagram(coordinateSystem(
            preserveAspectRatio=false,
            extent={{-100,-100},{100,100}},
            grid={2,2}), graphics={Line(points={{-80,-30},{-10,-30}}, color={0,
              0,255}),Line(points={{50,-30},{80,-30}}, color={0,0,255}),Line(
              points={{-80,30},{-50,30}}, color={0,0,255}),Line(points={{10,30},
              {80,30}}, color={0,0,255})}));
