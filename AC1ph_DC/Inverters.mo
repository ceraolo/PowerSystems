...
"));
  ...
  model Inverter "Complete modulator and inverter, 1-phase"
    ...
    replaceable model Inverter =
      ...
      "inverter model" annotation (choices(
...
<pre>
  ...
  model InverterAverage "Inverter time-average, 1-phase"
    ...
    parameter Integer modulation=1 "equivalent modulation :" annotation (
        Evaluate=true, choices(choice=1 "1: sine PWM", choice=3 "3: block M"));
  ...
  protected
    ...
    final parameter Real factor=if modulation == 1 then 1 else if modulation
         == 3 then (4/pi)*sin(width0*pi/2) else 0 annotation (Evaluate=true);
    SI.Angle phi;
    PS.Voltage Vloss;
    Real cT;
    Real hsw_nom;
  ...
  equation
    ...
    cT = if size(par.cT_loss, 1) > 0 then loss(T[1] - par.T0_loss, par.cT_loss)
       else 1;
    ...
    Q_flow = {par.eps[1]*R_nom*AC.i*AC.i + cT*(par.Vf + hsw_nom*abs(vDC1))*(abs(
      AC.i[1]) + abs(AC.i[2]))};
    annotation (
      defaultComponentName="inverter",
      Icon(coordinateSystem(
...
"));
  ...
  model Chopper "DC-DC converter"
    ...
    Modelica.Blocks.Interfaces.RealInput vDC "desired vDC" annotation (
        Placement(transformation(
          origin={60,100},
          extent={{-10,-10},{10,10}},
          rotation=270)));
    replaceable model Modulator = PowerSystems.Control.Modulation.ChopperPWM
      constrainedby PowerSystems.Icons.BlockS "modulator type" annotation (
        choicesAllMatching=true);
    Modulator modulator "modulator type"
      annotation (Placement(transformation(extent={{-10,40},{10,60}})));

    replaceable model Chopper =
        PowerSystems.AC1ph_DC.Inverters.Components.ChopperModular
      "chopper model" annotation (choicesAllMatching=true);
  ...
  equation
    ...
    connect(chopper.heat, heat) annotation (Line(points={{0,10},{0,20},{20,20},
            {20,80},{0,80},{0,100}}, color={176,0,0}));
    annotation (defaultComponentName="chopper", Documentation(info="<html>
...
<pre>
  ...
  model ChopperAverage "DC-DC converter time-average"
    ...
    Modelica.Blocks.Interfaces.RealInput uDC "desired uDC" annotation (
        Placement(transformation(
          origin={60,100},
          extent={{-10,-10},{10,10}},
          rotation=270)));
  protected
    ...
    function lossT = Utilities.Math.taylor "spec loss function of temperature";
  ...
  equation
    ...
    Q_flow = {par.eps[1]*R_nom*DCin.i[1]*DCin.i[1] + cT*(par.Vf + hsw_nom*abs(
      vDCin))*abs(DCin.i[1])};
    annotation (
      ...
      Diagram(coordinateSystem(
          ...
          grid={2,2}), graphics={Text(
              extent={{-40,-60},{40,-80}},
              lineColor={176,0,0},
              textString="time average equation"),Line(points={{-20,30},{-20,50}},
            color={0,0,255}),Line(points={{0,0},{0,20}}, color={0,0,255}),
            Polygon(
      ...
      annotation (
        ...
        Diagram(coordinateSystem(
            ...
            grid={2,2}), graphics={Text(
                  extent={{-40,-60},{40,-80}},
                  lineColor={176,0,0},
                  textString="time resolved equation"),Polygon(
              ...
              0},{60,0}}, color={0,0,255}),Line(points={{-70,10},{-60,10},{-60,
              52},{0,52},{0,40}}, color={0,0,255}),Line(points={{-70,-10},{-60,
              -10},{-60,-50},{0,-50},{0,-38}}, color={0,0,255})}),
        Documentation(info="<html>
...
"),     Icon(coordinateSystem(
    ...
    model InverterSwitch "Inverter equation, 1-phase"
      ...
      Modelica.Blocks.Interfaces.BooleanInput[4] gates
        "gates pairs {a1_p, a1_n, a2_p, a2_n}" annotation (Placement(
            transformation(
            origin={-60,100},
            extent={{-10,-10},{10,10}},
            rotation=270)));
    ...
    equation
      ...
      annotation (
        ...
        Diagram(coordinateSystem(
            preserveAspectRatio=false,
            extent={{-100,-100},{100,100}},
            grid={2,2}), graphics={Line(points={{0,0},{60,0}}, color={0,0,255}),
              Line(points={{0,-46},{0,46}}, color={0,0,255}),Polygon(
...
"));
    ...
    equation
      ...
      if par.Vf < 1e-3 then
        // faster code if forward voltage drop Vf not used (Vf_s=0 and Vf_d=0).
        for k in 1:2 loop
          V_s[k] = 0;
          V_d[k] = vDC1;
          if gates[pgt[k]] then
      ...
      annotation (
        ...
        Diagram(coordinateSystem(
            ...
            grid={2,2}), graphics={Text(
                  extent={{-40,-60},{40,-80}},
                  lineColor={176,0,0},
                  textString="time resolved equation"),Line(points={{30,-46},{
              30,46}}, color={0,0,255}),Line(points={{20,-14},{40,-14}}, color=
              {0,0,255}),Line(points={{20,34},{40,34}}, color={0,0,255}),Line(
              points={{-30,0},{60,0}}, color={0,0,255}),Polygon(
                  points={{20,14},{30,34},{40,14},{20,14}},
                  lineColor={0,0,255},
                  fillColor={255,255,255},
                  fillPattern=FillPattern.Solid),Polygon(
...
Blocking losses are neglected in the expression of dissipated heat <tt>Q_flow</tt>.</p>
    ...
    equation
      ...
      connect(gates, gate2demux1.gates) annotation (Line(points={{-60,100},{-60,
              80},{-40,80}}, color={255,0,255}));
      connect(AC, pn_p_n.term_pn)
        annotation (Line(points={{100,0},{76,0}}, color={0,0,255}));
      connect(pn_p_n.term_p, switchMod_a1.AC) annotation (Line(points={{64,4},{
              40,4},{40,30},{10,30}}, color={0,0,255}));
      ...
      connect(switchMod_a2.DC, DC) annotation (Line(points={{-10,-30},{-60,-30},
              {-60,0},{-100,0}}, color={0,0,255}));
      connect(switchMod_a1.heat, heat_adapt.port_a) annotation (Line(points={{0,
              40},{0,54},{-4,54},{-4,64}}, color={176,0,0}));
      connect(switchMod_a2.heat, heat_adapt.port_b) annotation (Line(points={{0,
              -20},{0,-10},{20,-10},{20,54},{4,54},{4,64}},color={176,0,0}));
      connect(heat_adapt.port_ab, heat)
        annotation (Line(points={{0,76},{0,100}}, color={176,0,0}));
      annotation (
        defaultComponentName="inverter",
    ...
    model ChopperModular "DC_DC converter modular"
      ...
      final parameter Data par "SC parameters"
        annotation (Placement(transformation(extent={{-80,-80},{-60,-60}})));
      Modelica.Blocks.Interfaces.BooleanInput gate "gate" annotation (Placement(
            transformation(
            origin={-60,100},
            extent={{-10,-10},{10,10}},
            rotation=270)));
      ...
      SCpackage.SCswitch_Diode switch_D(par=par) "switch + reverse diode"
        annotation (Placement(transformation(extent={{-10,20},{10,40}})));
      SCpackage.Diode diode(par=par) "diode" annotation (Placement(
            transformation(
            origin={30,-10},
            extent={{-10,-10},{10,10}},
            rotation=90)));
      ...
      Common.Thermal.Heat_a_b_ab heat_adapt
        annotation (Placement(transformation(extent={{-10,60},{10,80}})));
    equation
      connect(gate, switch_D.gate) annotation (Line(points={{-60,100},{-60,50},
              {6,50},{6,40}}, color={255,0,255}));
      connect(DCin, pn_p_n.term_pn)
        annotation (Line(points={{-100,0},{-76,0}}, color={0,0,255}));
      connect(pn_p_n.term_p, switch_D.term_p) annotation (Line(points={{-64,4},
              {-40,4},{-40,30},{-10,30}},color={0,0,255}));
      connect(switch_D.term_n, p_n_pn.term_p)
        annotation (Line(points={{10,30},{64,30},{64,4}}, color={0,0,255}));
      connect(pn_p_n.term_n, p_n_pn.term_n) annotation (Line(points={{-64,-4},{
              -40,-4},{-40,-32},{64,-32},{64,-4}}, color={0,0,255}));
      connect(p_n_pn.term_n, diode.term_p) annotation (Line(points={{64,-4},{64,
              -32},{30,-32},{30,-20}},color={0,0,255}));
      connect(diode.term_n, p_n_pn.term_p) annotation (Line(points={{30,0},{30,
              30},{64,30},{64,4}}, color={0,0,255}));
      ...
      connect(diode.heat, heat_adapt.port_b)
        annotation (Line(points={{20,-10},{20,64},{4,64}}, color={176,0,0}));
      connect(heat_adapt.port_ab, heat)
        annotation (Line(points={{0,76},{0,100}}, color={176,0,0}));
      annotation (
        defaultComponentName="chopper",
...
<li>Modular: composed from semiconductor-switches and diodes. These components with ideal V-I characteristic can be replaced by custom-specified semiconductor models.</li>
  ...
  package Partials "Partial models"
    ...
    partial model AC_DC_base "AC-DC base, 1-phase"
      ...
      Interfaces.ThermalV_n heat(m=2) "vector heat port" annotation (Placement(
            transformation(
            origin={0,100},
            extent={{-10,-10},{10,10}},
            rotation=90)));
      annotation (Icon(coordinateSystem(
            ...
            grid={2,2}), graphics={Text(
                  extent={{-70,34},{-10,4}},
                  lineColor={0,0,255},
                  textString="="),Line(points={{-80,-60},{80,60}}, color={0,0,
              255}),Text(extent={{0,-6},{80,-36}}, textString="~")}),
          Documentation(info="<html>
</html>"));
...
"), Icon(coordinateSystem(
            preserveAspectRatio=false,
            extent={{-100,-100},{100,100}},
            grid={2,2}), graphics={Line(points={{-80,-60},{80,60}}, color={0,0,
              255}),Text(
                  extent={{-70,34},{-10,4}},
                  lineColor={0,0,255},
                  textString="="),Text(extent={{0,-6},{80,-36}}, textString="="),
              Text(
                  extent={{-140,40},{-60,20}},
                  lineColor={0,0,255},
                  textString="in"),Text(
    ...
    protected
      ...
      function loss = Utilities.Math.taylor "spec loss function of temperature";
